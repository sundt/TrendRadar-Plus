...
if [ "${restart}" = "true" ]; then
  if [ -f docker/docker-compose.yml ] || [ -f docker/docker-compose.build.yml ] || [ -f docker/docker-compose-build.yml ]; then
    ...
    if docker compose version >/dev/null 2>&1; then
      docker compose restart trend-radar-viewer || docker compose restart
...
files=()
for p in trendradar docker mcp_server openspec README.md; do
  if [ -e "${p}" ]; then
    files+=("${p}")
  fi
done
...
if [ ! -f "./trendradar/web/templates/viewer.html" ]; then
  echo "❌ viewer.html not found: ./trendradar/web/templates/viewer.html"
  exit 1
fi
...
if [ "$DRY_RUN" = "true" ]; then
  python3 ./predeploy-cache-bust.py --file ./index.html --file ./trendradar/web/templates/viewer.html --dry-run
else
  python3 ./predeploy-cache-bust.py --file ./index.html --file ./trendradar/web/templates/viewer.html
fi
...
if [ -f docker/docker-compose.yml ] || [ -f docker/docker-compose.build.yml ] || [ -f docker/docker-compose-build.yml ]; then
  ...
  if docker compose version >/dev/null 2>&1; then
    if [ -f docker-compose-build.yml ]; then
      docker compose -f docker-compose-build.yml build trend-radar-viewer
    elif [ -f docker-compose.build.yml ]; then
      docker compose -f docker-compose.build.yml build trend-radar-viewer
    fi

    if [ -f docker-compose.yml ]; then
      docker compose -f docker-compose.yml up -d --force-recreate --no-deps trend-radar-viewer
    else
      docker compose up -d --force-recreate --no-deps trend-radar-viewer
    fi
  elif command -v docker-compose >/dev/null 2>&1; then
    if [ -f docker-compose-build.yml ]; then
      docker-compose -f docker-compose-build.yml build trend-radar-viewer
  ...
  systemctl restart trendradar || true
else
