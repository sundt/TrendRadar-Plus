      ...
      tr.innerHTML = `
              ...
              <td>
                <div class="row-actions">
                  <button class="btn btn-icon" title="Edit" onclick='editNewsNowPlatform(${JSON.stringify(p)})'>✏️</button>
                  <button class="btn btn-icon" title="${p.enabled ? 'Disable' : 'Enable'}" onclick="toggleNewsNow('${p.id}')">
                    ${p.enabled ? '✅' : '⛔'}
                  </button>
                  <button class="btn btn-icon" title="Delete" onclick="deleteNewsNow('${p.id}')" style="color:#dc2626;">🗑️</button>
                </div>
              </td>
            `;
