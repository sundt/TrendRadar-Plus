...
async function bulkSetCategorySelected() {
  const ids = getSelectedSourceIds();
  if (!ids.length) { alert('No rows selected.'); return; }

  // Convert to platform IDs (rss-123)
  const platformIds = ids.map(id => `rss-${id}`);

  if (typeof openBatchCategoryModal === 'function') {
    openBatchCategoryModal(platformIds);
  } else {
    alert('Unified platform module not loaded. Please switch to "Unified" tab once to load logic.');
  }
}

async function bulkSetScraperAPISelected(enableScraperAPI) {
  const ids = getSelectedSourceIds();
  if (!ids.length) { alert('请先选择源'); return; }
  ...
  _setCsvImportStatus(`批量${label} ScraperAPI 中...`);
  ...
  try {
    ...
    const resp = await fetch('/api/platform/batch-scraperapi', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Admin-Token': token
    ...
    _setCsvImportStatus(`✅ 已${label} ${ids.length} 个源的 ScraperAPI`);

    // Reload to reflect changes
    setTimeout(() => window.location.reload(), 1000);
  } catch (e) {
    _setCsvImportStatus(`❌ 操作失败: ${e.message}`);
    alert(e?.message || String(e));
  }
}

async function bulkSetSocksProxySelected(enableSocksProxy) {
  const ids = getSelectedSourceIds();
  if (!ids.length) { alert('请先选择源'); return; }

  // Convert to platform IDs (rss-123)
  const platformIds = ids.map(id => `rss-${id}`);
  const label = enableSocksProxy ? '启用' : '禁用';

  const ok = confirm(`批量${label} Socks5 代理？\n已选择: ${ids.length} 个源`);
  if (!ok) return;

  _setCsvImportStatus(`批量${label} Socks5 代理中...`);

  try {
    const payload = {
      platform_ids: platformIds,
      use_socks_proxy: enableSocksProxy
    };

    const resp = await fetch('/api/platform/batch-socks-proxy', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Admin-Token': token
    ...
    _setCsvImportStatus(`✅ 已${label} ${ids.length} 个源的 Socks5 代理`);
...
function _catalogCategoryMatch(row, cat) {
  const c = String(cat || '').trim().toLowerCase();
  if (!c) return true;
  const rowCat = String(row?.dataset?.category || '').trim().toLowerCase();
  return rowCat.includes(c); // loose match or exact? Standard categories are single words usually.
}
...
function applyCatalogFilter() {
  ...
  const rows = _getCatalogRows();
  for (const tr of rows) {
    const ok = _catalogFilterMatch(tr, _catalogFilter) && _catalogSearchMatch(tr, q) && _catalogCategoryMatch(tr, cat);
...
async function warmupSource(btnOrSourceId) {
  ...
  try {
    ...
    // Silently clear backoff first to ensure immediate run (User expectation: Run means Run)
    await fetch('/api/admin/rss-sources/clear-backoff', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token },
      body: JSON.stringify({ source_id: sid, warmup: 0 })
    }).catch(() => { });

    const resp = await fetch('/api/rss-sources/warmup?wait_ms=20000', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Admin-Token': token
    ...
    if (res0) {
      if (res0.ok === false) {
        throw new Error(String(res0.error || 'Unknown error'));
      }
      if (res0.queued) {
        // Timed out but queued
        _setCsvImportStatus(`抓取请求已入队: ${sid}`);
        if (btn) btn.textContent = '已入队';
      } else {
        // Completed successfully
        _setCsvImportStatus(`抓取完成: ${sid}`);
        if (btn) btn.textContent = '抓取成功';
        setTimeout(() => { if (btn) btn.textContent = '▶️'; }, 3000); // Reset button after 3s
      }
    } else {
...
function openEditModal(btn) {
  ...
  document.getElementById('edit-use-scraperapi').checked = (btn.dataset.useScraperapi === '1');
  document.getElementById('edit-use-socks-proxy').checked = (btn.dataset.useSocksProxy === '1');
...
async function saveEditSource() {
  ...
  const payload = {
    ...
    use_scraperapi: document.getElementById('edit-use-scraperapi').checked ? 1 : 0,
    use_socks_proxy: document.getElementById('edit-use-socks-proxy').checked ? 1 : 0
  };
...
async function quickAddRss() {
  ...
  try {
    const btn = document.querySelector('button[onclick="quickAddRss()"]');
    if (btn) btn.disabled = true;

    showToast("正在预览 RSS...", "info");
    ...
    if (!previewResp.ok) throw new Error(previewData.detail || 'Preview failed');

    if (!previewData.preview_hash) throw new Error('No preview hash returned');

    // check validity
    if (previewData.summary && previewData.summary.invalid > 0) {
      console.error("Invalid rows:", previewData.invalid_rows);
      throw new Error('RSS URL preview returned invalid or unreachable source.');
    }

    showToast("正在提交并保存...", "info");

    // 2. Commit
    const commitResp = await fetch(`/api/admin/rss-sources/import-csv/commit?token=${encodeURIComponent(token)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ csv_text: url, preview_hash: previewData.preview_hash })
    });
    const commitData = await commitResp.json();
    if (!commitResp.ok) throw new Error(commitData.detail || 'Commit failed');

    showToast("添加成功！正在刷新...", "success");
