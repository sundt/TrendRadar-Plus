...
from hotnews.storage.base import NewsData, NewsItem
...
def _is_custom_source(platform_id: str) -> bool:
    """Check if a platform ID belongs to a custom source."""
    prefixes = ("custom_", "owent_", "qborfy_", "maie_", "kotaku_")
    return any(platform_id.startswith(p) for p in prefixes)


def _save_custom_items_to_rss_entries(
    db_conn,
    platform_id: str,
    items: List[NewsItem],
    now_ts: int
    ...
    """
    Save custom source items to rss_entries table (unified storage).
    ...
    for item in items:
        try:
            # Use URL as dedup_key, fallback to title hash
            if item.url:
                dedup_key = item.url
            else:
                import hashlib
                dedup_key = hashlib.md5(item.title.encode()).hexdigest()
            
            # Insert or ignore (dedup by source_id + dedup_key)
            db_conn.execute("""
                INSERT OR IGNORE INTO rss_entries 
                (source_id, dedup_key, url, title, published_at, fetched_at, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                platform_id,
                dedup_key,
                item.url or "",
                item.title,
                item.published_at if hasattr(item, 'published_at') and item.published_at else now_ts,
                now_ts,
                now_ts
            ))
    ...
    try:
        ...
        for item in items:
            ...
            dedup.check_and_handle(platform_id, dedup_key, item.title, item.url or "", pub_at, dry_run=False)
    except Exception:
        pass
...
def run_provider_ingestion_once(
    *,
    ...
    # Load config, fallback to empty dict if file doesn't exist
    try:
    ...
    # Separate custom sources from mainstream platforms
    now_ts = int(time.time())
    custom_items = {}
    mainstream_items = {}
    
    for platform_id, items in items_by_id.items():
        if _is_custom_source(platform_id):
            custom_items[platform_id] = items
        else:
            mainstream_items[platform_id] = items
    
    # Save custom source items to rss_entries (unified storage)
    if custom_items and db_conn:
        for platform_id, items in custom_items.items():
            inserted = _save_custom_items_to_rss_entries(db_conn, platform_id, items, now_ts)
            print(f"[Custom] Saved {inserted} items for {platform_id} to rss_entries")
    
    # Save mainstream platform items to news.db (original behavior)
    if mainstream_items:
        mainstream_id_to_name = {k: v for k, v in id_to_name.items() if k in mainstream_items}
        data = NewsData(
            date=crawl_date,
            crawl_time=crawl_time,
            items=mainstream_items,
            id_to_name=mainstream_id_to_name,
