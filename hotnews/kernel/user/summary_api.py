...
async def generate_summary_stream(
    ...
    if row and row[0]:
        # Return cached summary as single SSE event
    ...
    async def stream_generator():
        ...
        async for chunk, is_done, a_type, err in generate_smart_summary_stream(content, api_key, model):
            if err:
            ...
            if chunk is None and not is_done:
                ...
                yield f"data: {json.dumps({'type': 'done', 'news_id': news_id, 'remaining': remaining - 1}, ensure_ascii=False)}\n\n"
