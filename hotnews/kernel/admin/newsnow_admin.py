...
async def list_newsnow_platforms(request: Request, _=Depends(_require_admin)):
    ...
    cur = conn.execute(
        """SELECT id, name, category, enabled, sort_order, category_override,
    ...
    for r in rows:
        ...
        results.append({
            "id": platform_id,
            "name": platform_name,
            "category": r[2] or "",  # Old field for backward compatibility
            "enabled": bool(r[3]),
            "sort_order": r[4] or 0,
            "category_override": r[5] or "",  # Manual override
            "auto_category": auto_category,     # Auto-assigned by rules
            "final_category": r[5] or auto_category,  # Final computed category
            "last_fetch_at": r[6],
            "last_status": r[7],
            "last_error": r[8],
            "updated_at": r[9]
...
async def update_newsnow_platform(platform_id: str, platform: NewsNowPlatform, request: Request, _=Depends(_require_admin)):
    ...
    try:
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # Use category as override (manual assignment)
        conn.execute(
            """UPDATE newsnow_platforms 
               SET name = ?, category_override = ?, enabled = ?, sort_order = ?, updated_at = ?
               WHERE id = ?""",
