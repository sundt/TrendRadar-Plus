...
from hotnews.web.rss_proxy import rss_proxy_fetch_cached
from hotnews.web.user_db import added_counts, get_user_db_conn, subscriber_counts
...
async def admin_login_page(request: Request):
    ...
    if is_password_auth_enabled():
        session_token = request.cookies.get(get_session_cookie_name(), "")
        if session_token:
            is_valid, _ = verify_admin_session(session_token)
            if is_valid:
                from fastapi.responses import RedirectResponse
                return RedirectResponse(url="/admin/rss-sources", status_code=302)
...
async def admin_login_submit(request: Request):
    ...
    password = str(body.get("password", "") if isinstance(body, dict) else "").strip()
    
    if not is_password_auth_enabled():
        return JSONResponse(
            content={"success": False, "message": "密码认证未启用，请使用Token方式"},
            status_code=400
        )
    ...
    response = JSONResponse(content={"success": True, "redirect": "/admin/rss-sources"})
    response.set_cookie(
...
async def admin_logout(request: Request):
    """Handle admin logout."""
    from hotnews.kernel.admin.admin_auth import get_session_cookie_name
    
    response = JSONResponse(content={"success": True, "redirect": "/admin/login"})
...
async def api_rss_sources_warmup(request: Request, wait_ms: int = Query(0)):
    ...
    if wait_ms and wait_ms > 0 and futures:
        timeout_s = min(60.0, max(0.0, float(wait_ms) / 1000.0))
...
async def admin_rss_sources_page(request: Request):
    from hotnews.kernel.admin.admin_auth import is_password_auth_enabled
    
    # Check if password auth is enabled and handle redirect
    try:
        _require_admin(request)
    except HTTPException as e:
        if e.status_code == 403 and is_password_auth_enabled():
    ...
    cur = conn.execute(
        "SELECT id, name, url, host, category, feed_type, country, language, source, seed_last_updated, enabled, created_at, updated_at, added_at, fail_count, backoff_until, last_error_reason, last_attempt_at, use_scraperapi, use_socks_proxy FROM rss_sources ORDER BY updated_at DESC"
    )
        ...
        sources.append(
            {
                ...
                "use_scraperapi": bool(int(r[18] or 0)),
                "use_socks_proxy": bool(int(r[19] or 0)),
            }
        )
    ...
    # Get platform categories for dropdown
    try:
        platform_categories = []
        cur_cat = conn.execute("SELECT id, name, icon, sort_order FROM platform_categories WHERE enabled = 1 ORDER BY sort_order, id")
        for r in cur_cat.fetchall():
            platform_categories.append({
                "id": str(r[0]),
                "name": str(r[1]),
                "icon": str(r[2])
            })
...
class UpdateSourceReq(BaseModel):
    ...
    use_scraperapi: int | None = None
    use_socks_proxy: int | None = None
...
async def api_admin_update_source(request: Request, req: UpdateSourceReq):
    ...
    if req.use_scraperapi is not None:
        fields.append("use_scraperapi = ?")
        values.append(1 if req.use_scraperapi else 0)

    if req.use_socks_proxy is not None:
        fields.append("use_socks_proxy = ?")
        values.append(1 if req.use_socks_proxy else 0)
...
async def api_admin_tool_fetch_html(request: Request, url: str):
    ...
    # SSL Verification: Default to True for security, configurable via env
    ssl_verify = os.environ.get("HOTNEWS_SSL_VERIFY", "true").lower() == "true"
    
    try:
        import requests
        headers = {
            "User-Agent": "Hotnews/1.0 (VisualSelector)",
        }
        import traceback
        ...
        <script>
            function getCssSelector(el) {
                ...
                while (el.nodeType === Node.ELEMENT_NODE) {
                    ...
                    if (el.id) {
                        ...
                        if (el.classList.length > 0 && !el.classList.contains('hotnews-highlight')) {
                            // cleanup highlighting classes
                            let cls = Array.from(el.classList).filter(c => c !== 'hotnews-highlight').join('.');
            ...
            document.addEventListener('mouseover', function(e) {
                e.stopPropagation();
                // Remove existing
                document.querySelectorAll('.hotnews-highlight').forEach(el => el.classList.remove('hotnews-highlight'));
                // Add new
                e.target.classList.add('hotnews-highlight');
            }, true);
            
            document.addEventListener('mouseout', function(e) {
                e.stopPropagation();
                e.target.classList.remove('hotnews-highlight');
            }, true);
