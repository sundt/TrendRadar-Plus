...
from hotnews.kernel.providers.runner import run_provider_ingestion_once, build_default_registry, ProviderIngestionConfig
...
# Search Index Update (Vector daily rebuild)
_search_vector_task: Optional[asyncio.Task] = None
_search_vector_running: bool = False
_search_vector_last_rebuild_date: str = ""

# Tag Auto-Promotion (Daily check)
_tag_promotion_task: Optional[asyncio.Task] = None
_tag_promotion_running: bool = False
...
def _mb_ai_budget_allow() -> bool:
    max_per_hour = 200
    try:
        max_per_hour = int(os.environ.get("HOTNEWS_MB_AI_MAX_PER_HOUR", "200"))
    except Exception:
        max_per_hour = 200
...
def _mb_ai_prompt_text() -> str:
    ...
    return (
        ...
        "• deep_dive: 长文分析、研报、深度解读、技术原理\n"
        "• breaking: 包含'突发'、'刚刚'、'快讯'、'速报'\n"
...
def _mb_ai_call_qwen(items: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    Uses the model pool instead of fixed qwen-plus model.
    Returns a list of outputs (same length/order), or raises.
    """
    
    # 1. Prepare prompt and user content
    messages = [
        {"role": "system", "content": _mb_ai_prompt_text()},
        {"role": "user", "content": json.dumps(items, ensure_ascii=False)},
    ]
    
    # 2. Call via Manager (uses model pool with auto-rotation)
    mgr = AIModelManager.get_instance()
    # Hack: ensure root is set if not already (might need a better place for this global init)
    if mgr._project_root is None:
        mgr.set_project_root(os.getcwd())
        
    try:
        data = mgr.call_chat_completion(messages)
    ...
    try:
        ...
        raise RuntimeError("ai_invalid_json_format")

    if not isinstance(parsed, list):
        raise RuntimeError("ai_invalid_json_not_array")
        
    # Allow slightly fewer items (max 20% loss)
...
def _mb_ai_normalize_row(out: Any) -> Dict[str, Any]:
    ...
    # Parse attributes (list, max 2)
    attrs_raw = out.get("attributes") or []
    if isinstance(attrs_raw, str):
        attrs_raw = [attrs_raw]
    attributes = []
    for a in attrs_raw[:2]:
        if isinstance(a, str) and a.strip():
            attributes.append(a.strip().lower())
    
    # Parse suggested_tags (list of dicts for dynamic tag discovery)
    suggested_tags_raw = out.get("suggested_tags") or []
    suggested_tags = []
...
def _mb_ai_select_unlabeled(conn, limit: int) -> List[Dict[str, Any]]:
    ...
    # 1. Fetch Morning Brief Rules to check for whitelist
    whitelist_enabled = False
    allowed_cats = set()
    try:
        cur_rules = conn.execute("SELECT value FROM admin_kv WHERE key='morning_brief_rules_v1'")
        row = cur_rules.fetchone()
        if row and row[0]:
            rules = json.loads(row[0])
            if isinstance(rules, dict):
                whitelist_enabled = bool(rules.get("category_whitelist_enabled", False))
                raw_list = rules.get("category_whitelist", [])
                if isinstance(raw_list, list):
                    allowed_cats = {str(x).strip() for x in raw_list if x}
    except Exception:
        pass
...
async def _mb_ai_loop() -> None:
    ...
    while _mb_ai_running:
        try:
            ...
            batch_size = 20
            try:
                batch_size = int(os.environ.get("HOTNEWS_MB_AI_BATCH_SIZE", "20"))
            except Exception:
                batch_size = 20
            ...
            check_interval = 30
            try:
                check_interval = int(os.environ.get("HOTNEWS_MB_AI_CHECK_INTERVAL", "30"))
            except Exception:
                check_interval = 30
            ...
            # Use the actual model name for logging/storage if available
...
async def mb_ai_run_once(batch_size: int = 20, force: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use the actual model name from the pool
        if used_model_name and used_model_name != "unknown":
            model = used_model_name
...
def _rss_source_ai_enabled() -> bool:
    ...
    try:
        enabled = (os.environ.get("HOTNEWS_RSS_SOURCE_AI_ENABLED") or "0").strip().lower() in {"1", "true", "yes"}
        if not enabled:
            return False
        key = (os.environ.get("DASHSCOPE_API_KEY") or "").strip()
        return bool(key)
...
async def _rss_source_ai_classify_call(items: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    """Call AI to classify RSS sources via AIModelManager"""
    ...
    # 2. Call via Manager (async wrapper)
    mgr = AIModelManager.get_instance()
    if mgr._project_root is None:
        mgr.set_project_root(os.getcwd())

    try:
        # call_chat_completion is sync, so we wrap it
        data = await asyncio.to_thread(mgr.call_chat_completion, messages)
    except Exception as e:
        raise RuntimeError(f"AI Call Failed: {e}")
...
async def _rss_source_ai_classify_loop() -> None:
    ...
    while _rss_source_ai_running:
        try:
            ...
            batch_size = 20
            try:
                batch_size = int(os.environ.get("HOTNEWS_RSS_SOURCE_AI_BATCH_SIZE", "20"))
            except Exception:
                batch_size = 20
            batch_size = max(1, min(50, batch_size))
...
async def _custom_ingest_loop() -> None:
    """Loop for custom source ingestion with intelligent Cadence scheduling.
    
    Uses smart scheduler when available:
    - Prioritizes sources from source_stats table (smart scheduling)
    - Falls back to direct custom_sources query if no smart stats exist
    - Updates source_stats after each fetch for adaptive scheduling
    """
    ...
    while _custom_ingest_running:
        try:
            ...
            conn = get_online_db_conn(_project_root)
            
            # Try smart scheduler first
            smart_due_sources = []
            try:
                from hotnews.kernel.services.rss_smart_scheduler import get_due_sources
                smart_due_sources = get_due_sources(conn, now_ts, source_type="custom", limit=20)
            except Exception as e:
                logger.debug("custom_ingest: smart scheduler not available: %s", e)
            
            # Get due custom sources (fallback: those with next_due_at <= now)
            try:
                # Get sources that are due but NOT in smart scheduler yet
                smart_ids = {s["source_id"] for s in smart_due_sources}
                cursor = conn.execute("""
                    SELECT id, name, cadence, next_due_at 
                    FROM custom_sources 
                    WHERE enabled = 1 AND (next_due_at <= ? OR next_due_at = 0)
                """, (now_ts,))
                fallback_sources = [
                    row for row in cursor.fetchall()
                    if row[0] not in smart_ids
                ]
            except Exception as e:
                logger.error("custom_ingest: error fetching due sources: %s", e)
                await asyncio.sleep(60)
                continue
            
            # Combine smart + fallback sources
            all_due_ids = set()
            for s in smart_due_sources:
                all_due_ids.add(s["source_id"])
            for row in fallback_sources:
                all_due_ids.add(row[0])
            
            if not all_due_ids:
                # No sources due, check again in 60 seconds
                await asyncio.sleep(60)
                continue
            
            smart_count = len(smart_due_sources)
            fallback_count = len(fallback_sources)
            logger.info("custom_ingest: %d sources due (smart=%d, fallback=%d)", 
                       len(all_due_ids), smart_count, fallback_count)
            
            # Run ingestion for all enabled sources
            # The runner will handle each platform
            ...
            # Update stats and next_due_at for all processed sources
            for source_id in all_due_ids:
                try:
                    # Get source info for cadence fallback
                    cursor = conn.execute(
                        "SELECT cadence FROM custom_sources WHERE id = ?",
                        (source_id,)
                    )
                    row = cursor.fetchone()
                    cadence = row[0] if row else "P2"
                    
                    # Try to update via smart scheduler
                    smart_next_due = None
                    try:
                        from hotnews.kernel.services.rss_smart_scheduler import update_source_stats
                        # Note: We assume success here; in production, you'd check actual fetch result
                        stats = update_source_stats(
                            conn, source_id, 
                            source_type="custom", 
                            has_new_entries=True,  # Optimistic; real impl would check
                            error_message=""
                        )
                        smart_next_due = stats.get("next_due_at", 0)
                        logger.debug("custom_ingest: smart stats updated for %s, next_due=%s, cadence=%s",
                                    source_id, smart_next_due, stats.get("cadence"))
                    except Exception as e:
                        logger.debug("custom_ingest: smart scheduler update failed for %s: %s", source_id, e)
                    
                    # Update custom_sources.next_due_at
                    if smart_next_due and smart_next_due > now_ts:
                        next_due = smart_next_due
                    else:
                        # Fallback to fixed cadence
                        next_due = rss_next_due_at(now_ts, cadence or "P2")
                    
                    conn.execute(
                        "UPDATE custom_sources SET next_due_at = ? WHERE id = ?",
                        (next_due, source_id)
                    )
                except Exception as e:
                    logger.error("custom_ingest: error updating next_due_at for %s: %s", source_id, e)
...
def _rss_parse_published_ts(published_raw: str) -> int:
    """
    Parse RSS published timestamp from various formats.
    
    Limits future timestamps to 1 day to prevent "tomorrow's news" from appearing.
    Timezone-naive datetimes are treated as UTC (common RSS convention).
    """
    s = (published_raw or "").strip()
    if not s:
        return 0

    # Maximum allowed future time: 1 day (previously was 7 days, which caused "tomorrow's news" issues)
    MAX_FUTURE_SECONDS = 24 * 60 * 60
        ...
        if s.isdigit():
            ...
            if ts > 0:
                ...
                if ts > now + MAX_FUTURE_SECONDS:
                    logger.debug("rss_parse_ts.reject_future numeric ts=%s now=%s", ts, now)
                    return 0
                return ts
    except Exception:
        pass
    ...
    try:
        ...
        if dt.tzinfo is None:
            # Treat timezone-naive as UTC (common RSS convention)
            dt = dt.replace(tzinfo=timezone.utc)
        ts = int(dt.timestamp())
        now = _now_ts()
        if ts > now + MAX_FUTURE_SECONDS:
            logger.debug("rss_parse_ts.reject_future iso ts=%s now=%s raw=%s", ts, now, published_raw[:50])
            return 0
        return ts
    except Exception:
        pass
    ...
    try:
        ...
        now = _now_ts()
        if ts > now + MAX_FUTURE_SECONDS:
            logger.debug("rss_parse_ts.reject_future rfc822 ts=%s now=%s raw=%s", ts, now, published_raw[:50])
            return 0
        return ts
    except Exception:
        pass
...
def _update_fts_index_for_rss_entries(entries: List[tuple]) -> None:
    """
    Incrementally update FTS5 index with new RSS entries.
    
    Called immediately after RSS entries are committed to the database.
    This ensures new RSS content is searchable in near-realtime.
    
    Args:
        entries: List of tuples (source_id, dedup_key, url, title, published_at, published_raw, fetched_at, created_at)
    """
    if not entries:
        return
    try:
        ...
        if data:
            ...
            logger.debug("fts_incremental.rss entries=%s", len(data))
    except Exception as e:
        logger.warning("fts_incremental.rss fail error=%s", str(e)[:200])


async def _search_vector_rebuild_loop() -> None:
    """
    Daily scheduled task to rebuild vector index at 03:00.
    
    Vector index rebuilding is expensive (requires embedding model),
    so we do it once per day during low-traffic hours.
    """
    global _search_vector_running, _search_vector_last_rebuild_date
    
    logger.info("search_vector.scheduler.start")
    
    while _search_vector_running:
        try:
            now = datetime.now()
            today = now.strftime("%Y-%m-%d")
            
            # Run at 03:00-03:05, once per day
            should_run = (
                now.hour == 3 and 
                now.minute < 5 and 
                _search_vector_last_rebuild_date != today
            )
            
            if should_run:
                logger.info("search_vector.rebuild.start")
                try:
                    from hotnews.search import get_search_manager
                    manager = get_search_manager()
                    # Force rebuild includes both FTS and vector
                    await asyncio.to_thread(manager.build_all_indexes, force=True)
                    _search_vector_last_rebuild_date = today
                    logger.info("search_vector.rebuild.ok")
                except Exception as e:
                    logger.warning("search_vector.rebuild.fail error=%s", str(e)[:500])
                
                # Wait 1 hour after rebuild to avoid running multiple times
            ...
            logger.warning("search_vector.loop error=%s", str(e)[:200])
            await asyncio.sleep(60)
    
    logger.info("search_vector.scheduler.stop")


def _rss_entries_retention_cleanup(conn, source_id: str, now_ts: int) -> None:
    """
    Clean up old RSS entries based on ingestion time (created_at).
    
    Uses system settings: retention.rss_entries_retention_days
    - If 0: keep forever (no time-based cleanup)
    - Otherwise: delete entries where created_at is older than N days
    
    Also keeps at most 500 entries per source regardless of age.
    """
    ...
    try:
        if _project_root:
            ...
            settings = get_system_settings(Path(_project_root))
            retention = settings.get("retention", {})
            retention_days = int(retention.get("rss_entries_retention_days", 90))
    except Exception:
        pass
    
    # Time-based cleanup: delete entries older than retention_days (based on created_at)
    if retention_days > 0:
        cutoff = int(now_ts) - retention_days * 24 * 60 * 60
        try:
            # Delete entries where:
            # - created_at is older than cutoff, AND
            # - if published_at exists and is valid, it must also be older than cutoff
            conn.execute(
                "DELETE FROM rss_entries WHERE (published_at > 0 AND published_at < ? AND created_at < ?) OR created_at < ?",
                (cutoff, cutoff, cutoff),
            )
        except Exception:
            pass
    
    # Per-source limit: keep at most 500 entries per source
    try:
        conn.execute(
            """
...
def rss_budget_allow(priority: int) -> bool:
    ...
    max_per_hour = 60
    try:
        max_per_hour = int(os.environ.get("HOTNEWS_RSS_WARMUP_MAX_PER_HOUR", "60"))
    except Exception:
        max_per_hour = 60
    now = time.time()
...
async def _rss_process_warmup_one(source_id: str) -> Dict[str, Any]:
    ...
    conn = _get_online_db_conn()
    cur = conn.execute(
        "SELECT id, url, enabled, cadence, etag, last_modified, fail_count, backoff_until, scrape_rules, use_scraperapi, use_socks_proxy FROM rss_sources WHERE id = ?",
        (sid,),
    )
    row = cur.fetchone()
    ...
    use_scraperapi = bool(int(row[9] or 0))
    use_socks_proxy = bool(int(row[10] or 0))
    try:
        # Use Async fetch (aiohttp) for better concurrency and timeout handling
        logger.info(f"DEBUG: scheduler calling fetch_async sid={sid}")
        fetched = await rss_proxy_fetch_warmup_async(url, etag, last_modified, scrape_rules, use_scraperapi, use_socks_proxy)
        logger.info(f"DEBUG: scheduler fetch_async returned sid={sid}")
        ...
        logger.info(f"DEBUG: scheduler updating success sid={sid}")
        ...
        has_new_entries = len(rows_to_insert) > 0 if 'rows_to_insert' in dir() else False
        try:
            from hotnews.kernel.services.rss_smart_scheduler import update_source_stats
            stats = update_source_stats(conn, sid, source_type="rss", has_new_entries=has_new_entries, error_message="")
            smart_next_due = stats.get("next_due_at", 0)
            if smart_next_due > 0:
                next_due = smart_next_due
        except Exception as smart_e:
            logger.debug("rss_warmup.smart_stats_update_error source_id=%s error=%s", sid, str(smart_e))
        ...
        logger.info(f"DEBUG: scheduler outer exception sid={sid} err={e}")
        try:
            fail_count = int(row[6] or 0) + 1
            reason = str(e)
        except Exception as parse_ex:
            logger.error(f"Failed to parse exception details sid={sid}: {parse_ex}", exc_info=True)
            fail_count = 1
            reason = str(e)
        
        try:
            # Auto-enable ScraperAPI on 403 or SSL/Connection errors if key is available and not already enabled
            # This feature is controlled by SCRAPERAPI_AUTO_ENABLE env var (default: disabled for manual control)
            scraperapi_auto_enabled = False
            auto_enable_allowed = os.environ.get("SCRAPERAPI_AUTO_ENABLE", "false").strip().lower() in {"true", "1", "yes"}
            
            if auto_enable_allowed:
                error_triggers = ["403", "Forbidden", "SSLError", "ConnectionResetError", "RemoteDisconnected", "UNEXPECTED_EOF_WHILE_READING", "Blocked", "Timeout", "timeout"]
                if not use_scraperapi and any(trigger in reason for trigger in error_triggers):
                    if os.environ.get("SCRAPERAPI_KEY", "").strip():
                        conn.execute("UPDATE rss_sources SET use_scraperapi = 1 WHERE id = ?", (sid,))
                        conn.commit()
                        scraperapi_auto_enabled = True
                        logger.info("rss_warmup.auto_enable_scraperapi source_id=%s reason=error_detected", sid)
            
            backoff_sec = rss_backoff_s(fail_count, reason)
            until = now + int(backoff_sec)
            
            # Auto-disable sources after too many consecutive failures (network unreachable, etc.)
            # Configurable via HOTNEWS_RSS_AUTO_DISABLE_THRESHOLD (default: 10)
            auto_disabled = False
            try:
                auto_disable_threshold = int(os.environ.get("HOTNEWS_RSS_AUTO_DISABLE_THRESHOLD", "10"))
            except Exception:
                auto_disable_threshold = 10
            
            # Network-related errors that indicate the source is likely unreachable
            network_errors = ["Network is unreachable", "Connection refused", "Timeout", "timeout", 
                              "ConnectionError", "SSLError", "ConnectionResetError", "RemoteDisconnected",
                              "Name or service not known", "getaddrinfo failed", "Upstream timeout"]
            is_network_error = any(err in reason for err in network_errors)
            
            if fail_count >= auto_disable_threshold and is_network_error:
                conn.execute(
                    "UPDATE rss_sources SET enabled = 0, fail_count = ?, backoff_until = ?, last_error_reason = ? WHERE id = ?",
                    (fail_count, until, f"[Auto-disabled] {reason[:480]}", sid),
                )
                auto_disabled = True
                logger.warning(
                    "rss_warmup.auto_disable source_id=%s fail_count=%s reason=%s",
                    sid, fail_count, reason[:200]
                )
            else:
                conn.execute(
            ...
            # Update smart scheduler stats on failure
            try:
                from hotnews.kernel.services.rss_smart_scheduler import update_source_stats
                stats = update_source_stats(conn, sid, source_type="rss", has_new_entries=False, error_message=reason)
                smart_next_due = stats.get("next_due_at", 0)
                if smart_next_due > 0:
                    conn.execute("UPDATE rss_sources SET next_due_at = ? WHERE id = ?", (smart_next_due, sid))
            except Exception as smart_e:
                logger.debug("rss_warmup.smart_stats_update_error source_id=%s error=%s", sid, str(smart_e))
            
            logger.info(f"DEBUG: rss_scheduler committing transaction sid={sid}")
            conn.commit()
            logger.info(f"DEBUG: rss_scheduler committed transaction sid={sid}")
            logger.warning(
                "rss_warmup.fetch fail source_id=%s fail_count=%s backoff_until=%s error=%s scraperapi_auto=%s auto_disabled=%s",
                sid,
                fail_count,
                until,
                reason,
                scraperapi_auto_enabled,
                auto_disabled,
            )
            return {"ok": False, "source_id": sid, "error": reason, "backoff_until": until, "scraperapi_auto_enabled": scraperapi_auto_enabled, "auto_disabled": auto_disabled}
        except Exception as handler_ex:
            logger.error(f"Exception handler failed sid={sid}: {handler_ex}", exc_info=True)
...
async def _rss_warmup_worker_loop() -> None:
    ...
    while _rss_warmup_running:
        ...
        try:
            ...
            logger.error(f"Worker Loop Critical Error sid={sid}: {e}", exc_info=True)
            try:
                if fut is not None and not fut.done():
                    fut.set_exception(e)
            except Exception:
    ...
    while _rss_warmup_running:
        try:
            ...
            try:
                ...
                if smart_due:
                    smart_due_count = len(smart_due)
                    logger.info("rss_warmup.producer smart_due_sources=%s", smart_due_count)
                    for src in smart_due:
                        sid = (src.get("source_id") or "").strip()
                        if sid:
                            await rss_enqueue_warmup(sid, priority=10)
            except Exception as e:
                logger.debug("rss_warmup.producer smart_scheduler_error=%s", str(e))
            
            # Fallback: also check sources without stats (new sources)
            cur = conn.execute(
                """
                SELECT s.id FROM rss_sources s
                LEFT JOIN source_stats st ON s.id = st.source_id
                WHERE s.enabled = 1 AND s.cadence != 'P7'
                AND st.source_id IS NULL
                AND (s.next_due_at = 0 OR s.next_due_at <= ?)
                AND (s.backoff_until = 0 OR s.backoff_until <= ?)
                ORDER BY s.next_due_at ASC LIMIT ?
                """,
                (now, now, max(0, 10 - smart_due_count)),
            )
            rows = cur.fetchall() or []
            if rows:
                logger.info("rss_warmup.producer fallback_due_sources=%s", len(rows))
            for r in rows:
                sid = (r[0] or "").strip()
                if not sid:
                    continue
...
async def start(app, project_root) -> None:
    ...
    global _search_vector_task, _search_vector_running
    ...
    enabled = (os.environ.get("HOTNEWS_RSS_WARMUP_ENABLED") or "1").strip().lower() not in {
        "0",
        "false",
        "no",
    }
    if not enabled:
        logger.info("rss_warmup.disabled HOTNEWS_RSS_WARMUP_ENABLED=%s", os.environ.get("HOTNEWS_RSS_WARMUP_ENABLED"))
        return
    ...
    logger.info("rss_warmup.start enabled=%s max_per_hour=%s", enabled, os.environ.get("HOTNEWS_RSS_WARMUP_MAX_PER_HOUR", "60"))
    ...
    # Search Vector Index Daily Rebuild
    _search_vector_running = True
    if _search_vector_task is None or _search_vector_task.done():
        _search_vector_task = asyncio.create_task(_search_vector_rebuild_loop())
        logger.info("search_vector.scheduler.enabled")
...
async def stop() -> None:
    ...
    try:
        if _search_vector_task is not None:
            _search_vector_task.cancel()
    except Exception:
        pass

    try:
        if _tag_promotion_task is not None:
            _tag_promotion_task.cancel()
    except Exception:
        pass


def get_all_tasks_status() -> Dict[str, Any]:
    """
    Get status of all internal scheduled tasks.
    """
    return {
        "rss_warmup": {
            "running": _rss_warmup_running,
            "worker_active": _rss_warmup_worker_task is not None and not _rss_warmup_worker_task.done(),
            "producer_active": _rss_warmup_producer_task is not None and not _rss_warmup_producer_task.done(),
            "config": {
                "enabled": (os.environ.get("HOTNEWS_RSS_WARMUP_ENABLED") or "1").strip().lower() not in {"0", "false", "no"},
                "max_per_hour": os.environ.get("HOTNEWS_RSS_WARMUP_MAX_PER_HOUR", "60"),
            }
        },
        "mb_ai": {
            "running": _mb_ai_running,
        ...
        "rss_source_ai": {
            "running": _rss_source_ai_running,
            "task_active": _rss_source_ai_task is not None and not _rss_source_ai_task.done(),
            "last_run_date": _rss_source_ai_last_run_date,
            "config": {
                "enabled": _rss_source_ai_enabled(),
            }
        },
        "custom_ingest": {
            "running": _custom_ingest_running,
            "task_active": _custom_ingest_task is not None and not _custom_ingest_task.done(),
        },
        "search_vector": {
            "running": _search_vector_running,
            "task_active": _search_vector_task is not None and not _search_vector_task.done(),
            "last_rebuild_date": _search_vector_last_rebuild_date,
        },
        "tag_promotion": {
            "running": _tag_promotion_running,
            "task_active": _tag_promotion_task is not None and not _tag_promotion_task.done(),
            "last_run_date": _tag_promotion_last_run_date,
        }
    }


# ========== Tag Auto-Promotion (Daily) ==========

async def _tag_promotion_loop() -> None:
    """
    ...
    - min_confidence: 0.7+ average
    - min_time_span_days: 3+ days since first seen
    """
    global _tag_promotion_running, _tag_promotion_last_run_date
    ...
    while _tag_promotion_running:
        try:
            ...
            if not qualified:
                logger.info("tag_promotion.no_qualified_candidates")
                _tag_promotion_last_run_date = today
                await asyncio.sleep(check_interval_seconds)
                continue
            
            # Auto-promote each qualified candidate
            promoted_count = 0
            ...
            _tag_promotion_last_run_date = today
            logger.info(f"tag_promotion.done promoted={promoted_count} candidates={len(qualified)}")
            
        except Exception as e:
            logger.warning(f"tag_promotion.error: {e}")
        
        await asyncio.sleep(check_interval_seconds)
    
    logger.info("tag_promotion.stop")


def start_tag_promotion_task() -> None:
    """Start the tag auto-promotion background task."""
    global _tag_promotion_task, _tag_promotion_running
    
    if _tag_promotion_running:
        return
    
    _tag_promotion_running = True
    _tag_promotion_task = asyncio.create_task(_tag_promotion_loop())
    logger.info("tag_promotion.task_started")


def stop_tag_promotion_task() -> None:
    """Stop the tag auto-promotion background task."""
    global _tag_promotion_task, _tag_promotion_running
    
    _tag_promotion_running = False
    if _tag_promotion_task is not None:
        try:
            _tag_promotion_task.cancel()
        except Exception:
            pass
    logger.info("tag_promotion.task_stopped")
