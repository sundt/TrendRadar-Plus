import{a as D,b as H,c as pe,d as A}from"./chunk-BKJ5NW3H.js";import"./chunk-2LJVOGXX.js";import{a as K,b as de,c as m}from"./chunk-I35K533E.js";import{a as Y}from"./chunk-5VHEYNGL.js";import{a as N,b as J}from"./chunk-N3WOZHU5.js";function Me(e){return new Promise(t=>{let o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:10010;display:flex;align-items:center;justify-content:center;";let c=document.createElement("div");c.style.cssText="background:#fff;border-radius:12px;padding:24px;max-width:320px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.2);text-align:center;",c.innerHTML=`
            ...
            </div>`,o.appendChild(c),document.body.appendChild(o);let n=s=>{o.remove(),t(s)};c.querySelector(".confirm-cancel").onclick=()=>n(!1),c.querySelector(".confirm-ok").onclick=()=>n(!0),o.addEventListener("click",s=>{s.target===o&&n(!1)})})}var I=[],w=null,d=null,Q=new Map,W=0;function oe(e){return Q.has(e)||Q.set(e,{loading:!1,loaded:!1}),Q.get(e)}function ce(e){Q.set(e,{loading:!1,loaded:!1})}function ge(){Q.clear()}var F=null,U=null,_=null,ye=null,R=null,E=null,v=null,V=null;function ue(){let t=N.getUser?.()?.id;document.querySelectorAll(".sub-tab.topic-tab").forEach(c=>{let n=c.dataset.ownerUserId,s=c.dataset.category;if(!t||n&&String(n)!==String(t)){console.warn(`[TopicTracker] Removing topic tab: ${s}, owner=${n}, current=${t}`),c.remove();let a=document.getElementById(`tab-${s}`);a&&a.remove()}})}function qe(){document.querySelectorAll(".sub-tab.topic-tab[data-owner-user-id]").forEach(t=>{let o=t.dataset.category;console.log(`[TopicTracker] Removing server-rendered topic tab: ${o}`),t.remove();let c=document.getElementById(`tab-${o}`);c&&c.remove()})}function se(){if(console.log("[TopicTracker] Initializing..."),!document.body){console.warn("[TopicTracker] document.body not ready, retrying..."),setTimeout(se,100);return}qe(),ue(),Be(),D.on("mainNav:topicsActivated",()=>{console.log("[TopicTracker] mainNav:topicsActivated event received, loading topics..."),z()}),D.on("tab:switched",t=>{let o=t?.categoryId;if(console.log("[TopicTracker] tab:switched event received, categoryId:",o),o&&String(o).startsWith("topic-")){let c=String(o).replace("topic-","");console.log("[TopicTracker] Topic tab switched, loading:",c),j(c)}}),D.on("viewer:rendered",()=>{console.log("[TopicTracker] viewer:rendered event received, resetting states and setting up listeners..."),W++,console.log("[TopicTracker] Generation bumped to:",W),ue(),ge(),ve(),z();let t=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(t){let o=t.id.replace("tab-topic-","");console.log("[TopicTracker] Active topic tab detected after re-render:",o),setTimeout(()=>j(o),100)}}),window.addEventListener("authStateChanged",t=>{console.log("[TopicTracker] Auth state changed, reloading topics..."),Ee(),t?.detail?.user&&z()});let e=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(e){let t=e.id.replace("tab-topic-","");console.log("[TopicTracker] Topic tab already active on page load:",t),setTimeout(()=>j(t),200)}}function Ee(){document.querySelectorAll(".sub-tab.topic-tab").forEach(e=>e.remove()),document.querySelectorAll('.tab-pane[id^="tab-topic-"]').forEach(e=>e.remove()),ge(),I=[]}async function z(){try{N.initialized||(console.log("[TopicTracker] Waiting for authState to initialize..."),await N.init())}catch(o){console.warn("[TopicTracker] authState init failed:",o)}if(!De()){console.log("[TopicTracker] User not logged in, skipping topic tabs loading");return}let e=document.getElementById("topicSubTabs");if(!e){console.warn("[TopicTracker] topicSubTabs not found");return}let t=Ce(e);try{console.log("[TopicTracker] Loading topics from API...");let c=await(await fetch("/api/topics",{credentials:"include"})).json();t&&t.remove(),c.ok&&c.topics?.length>0?(I=c.topics,console.log(`[TopicTracker] Loaded ${I.length} topics, rendering tabs...`),Le(I),_e()):(console.log("[TopicTracker] No topics found or API error:",c.detail||c.error),I=[])}catch(o){console.error("[TopicTracker] Failed to load topics:",o),t&&t.remove(),I=[]}}function _e(){try{let e=localStorage.getItem("hotnews_active_tab"),t=localStorage.getItem("tr_active_tab"),o=e&&e.startsWith("topic-")?e:t;if(o&&o.startsWith("topic-")){let c=document.querySelector(`.sub-tab[data-category="${o}"]`),n=document.getElementById(`tab-${o}`);if(c&&n)console.log(`[TopicTracker] Restoring to topic tab: ${o}`),A.switchTab(o);else{console.log(`[TopicTracker] Topic tab not found, clearing saved tab: ${o}`),localStorage.removeItem("tr_active_tab");try{H.consumeNavigationState?.()}catch{}A.switchTab("my-tags")}}}catch(e){console.error("[TopicTracker] Failed to restore topic tab:",e)}}function Ce(e){let t=e.querySelector(".sub-tabs-indicator"),o=document.createElement("div");o.className="topic-tabs-loading",o.id="topicTabsSkeleton",o.style.cssText="display:flex;gap:4px;";for(let n=0;n<2;n++){let s=document.createElement("div");s.className="topic-tab-skeleton",s.innerHTML='<div class="skeleton-text"></div>',o.appendChild(s)}let c=e.querySelector(".sub-tab-new");return c?e.insertBefore(o,c):t?e.insertBefore(o,t):e.appendChild(o),o}function Le(e){let t=document.getElementById("topicSubTabs");if(!t){console.warn("[TopicTracker] topicSubTabs not found");return}let o=t.querySelector(".sub-tabs-indicator");e.forEach(c=>{let n=`topic-${c.id}`;if(t.querySelector(`[data-category="${n}"]`)){console.log(`[TopicTracker] Tab ${n} already exists, skipping`);return}let s=document.createElement("button");s.className="sub-tab topic-tab",s.dataset.category=n,s.dataset.topicId=c.id,s.onclick=()=>{A.switchTab(n)},s.innerHTML=`
                ${m(c.name)}
            `;let a=t.querySelector(".sub-tab-new");t.insertBefore(s,a||null),Ae(c),console.log(`[TopicTracker] Created tab for topic: ${c.name}`)}),He(),A.updateIndicator&&A.updateIndicator(t)}function Ae(e){let t=`topic-${e.id}`,o=document.querySelector(".tab-content-area");if(!o){console.error("[TopicTracker] tab-content-area not found");return}if(document.getElementById(`tab-${t}`))return;let c=document.createElement("div");c.className="tab-pane",c.id=`tab-${t}`,c.dataset.lazyLoad="0",c.innerHTML=`
            <div class="platform-grid" id="topicCards-${e.id}" data-topic-id="${e.id}">
                ${Y(window.innerWidth<=640?1:3)}
            </div>
                ...
                <li class="news-item" data-news-id="${p}" data-news-title="${u}" data-news-url="${l}">
                    <div class="news-item-content">
                        ...
                        <div class="news-actions">
                            <span class="tr-news-date">${Ie(i.published_at)}</span>
        ...
        `}function Oe(e,t){let o=t.length>0?t.slice(0,50).map((n,s)=>{let a=n.id||`topic-${Date.now()}-${s}`,i=m(n.title||""),r=m(n.url||""),p=i.replace(/'/g,"\\'").replace(/"/g,"&quot;"),u=r.replace(/'/g,"\\'").replace(/"/g,"&quot;"),l=`topic-kw-${e}`,h=m(e).replace(/'/g,"\\'"),y=`<button class="news-summary-btn" data-news-id="${a}" data-title="${i}" data-url="${r}" data-source-id="${l}" data-source-name="${h}" onclick="event.preventDefault();event.stopPropagation();handleSummaryClick(event, '${a}', '${p}', '${u}', '${l}', '${h}')"></button>`,x=`<button class="news-comment-btn" data-url="${r}" data-title="${i}"></button>`;return`
                <li class="news-item" data-news-id="${a}" data-news-title="${i}" data-news-url="${r}">
                    <div class="news-item-content">
                        ...
                        <div class="news-actions">
                            <span class="tr-news-date">${Ie(n.published_at)}</span>
        ...
        `}function De(){return!!N.getUser()}async function ze(){if(J()){try{let t=await(await fetch("/api/subscription/status",{credentials:"include"})).json();if(t.ok!==!1&&!t.is_vip){typeof window.openSubscriptionModal=="function"?window.openSubscriptionModal():alert("\u65B0\u589E\u4E3B\u9898\u4E3A\u4F1A\u5458\u4E13\u5C5E\u529F\u80FD\uFF0C\u8BF7\u524D\u5F80\u8BBE\u7F6E\u4E2D\u5FC3\u5347\u7EA7\u4F1A\u5458\u3002");return}}catch(e){console.error("[TopicTracker] Failed to check subscription status:",e)}w=null,d=null,U.textContent="\u65B0\u5EFA\u8FFD\u8E2A\u4E3B\u9898",_.value="",document.getElementById("topicIconGroup").style.display="none",document.getElementById("topicKeywordsGroup").style.display="none",document.getElementById("topicSourcesGroup").style.display="none",E.style.display="",E.disabled=!1,E.innerHTML="\u{1F916} \u751F\u6210\u5173\u952E\u8BCD\u548C\u63A8\u8350\u6E90",v.style.display="none",F.classList.add("active"),_.focus()}}function X(){F.classList.remove("active"),w=null,d=null}function me(e,t){return new Promise(o=>{let c=`
        ...
        `,document.body.appendChild(e),Z()}var q={polling:!1,sessionId:null};async function Z(){let e=document.getElementById("qrcodeModalQRArea");if(e){e.innerHTML=`
            <div class="topic-wechat-qr-loading">
                <span class="topic-spinner"></span>
                \u6B63\u5728\u83B7\u53D6\u4E8C\u7EF4\u7801...
            </div>
        `;try{let o=await(await fetch("/api/wechat/auth/qr/start",{method:"POST",credentials:"include"})).json();if(!o.ok)throw new Error(o.error||"\u521B\u5EFA\u4F1A\u8BDD\u5931\u8D25");q.sessionId=o.session_id;let c=`/api/wechat/auth/qr/image?t=${Date.now()}`;e.innerHTML=`
            ...
            `,q.polling=!0,ee()}catch(t){console.error("[TopicTracker] Start QR error:",t),e.innerHTML=`
            ...
            `}}}async function ee(){if(q.polling)try{let t=await(await fetch("/api/wechat/auth/qr/status",{credentials:"include"})).json(),o=document.getElementById("qrcodeModalStatus");if(!o){q.polling=!1;return}if(t.status==="waiting")o.textContent="\u7B49\u5F85\u626B\u7801...",o.className="wechat-qrcode-status";else if(t.status==="scanned")o.textContent="\u5DF2\u626B\u7801\uFF0C\u8BF7\u5728\u624B\u673A\u4E0A\u786E\u8BA4\u767B\u5F55",o.className="wechat-qrcode-status scanned";else if(t.status==="confirmed"){o.textContent="\u2705 \u6388\u6743\u6210\u529F\uFF01",o.className="wechat-qrcode-status confirmed",q.polling=!1,await Je();return}else if(t.status==="expired"||t.need_refresh){o.textContent="\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F",o.className="wechat-qrcode-status expired",q.polling=!1;let c=document.getElementById("qrcodeModalQRArea");c&&(c.innerHTML=`
                    ...
                    `);return}else t.status==="error"&&(o.textContent=t.message||"\u51FA\u9519\u4E86",o.className="wechat-qrcode-status error");q.polling&&setTimeout(ee,2e3)}catch(e){console.error("[TopicTracker] Poll QR status error:",e),q.polling&&setTimeout(ee,3e3)}}async function Je(){try{let t=await(await fetch("/api/wechat/auth/qr/complete",{method:"POST",credentials:"include"})).json();if(t.ok){let o=document.getElementById("qrcodeModalStatus");o&&(o.textContent="\u2705 \u6388\u6743\u6210\u529F\uFF01\u6B63\u5728\u751F\u6210...",o.className="wechat-qrcode-status confirmed"),Ge(),setTimeout(()=>{ne(),Te()},1e3)}else{let o=document.getElementById("qrcodeModalStatus");o&&(o.textContent=t.error||"\u767B\u5F55\u5931\u8D25",o.className="wechat-qrcode-status error")}}catch(e){console.error("[TopicTracker] Complete QR login error:",e)}}function Ye(){q.polling=!1,Z()}function ne(){let e=document.getElementById("wechatQrcodeModal");e&&e.classList.remove("active")}function Ve(){ne(),be()}function we(){d&&(document.getElementById("topicIconGroup").style.display="block",document.getElementById("topicIconPreview").textContent=d.icon,document.getElementById("topicKeywordsGroup").style.display="block",ae(),d.recommended_sources.length>0&&(document.getElementById("topicSourcesGroup").style.display="block",ie()))}function ae(){if(!d)return;let e=d.keywordStats||{},t=d.keywords.map(o=>{let c=e[o],n=c!==void 0,s=c>0,a="";return n&&(s?a=`<span class="topic-keyword-count has-news">${c}</span>`:a='<span class="topic-keyword-count no-news" title="\u6682\u65E0\u5339\u914D\u65B0\u95FB">0</span>'),`
            ...
            `}).join("")+`
            <button class="topic-keyword-add" onclick="TopicTracker.addKeyword()">+ \u6DFB\u52A0\u5173\u952E\u8BCD</button>
        `;ye.innerHTML=t}function Xe(){let e=prompt("\u8BF7\u8F93\u5165\u5173\u952E\u8BCD\uFF1A");!e||!e.trim()||(d||(d={icon:"\u{1F3F7}\uFE0F",keywords:[],recommended_sources:[]},document.getElementById("topicIconGroup").style.display="block",document.getElementById("topicKeywordsGroup").style.display="block"),d.keywords.includes(e.trim())||(d.keywords.push(e.trim()),ae(),v.disabled=!1))}function Ze(e){d&&(d.keywords=d.keywords.filter(t=>t!==e),ae(),d.keywords.length===0&&(v.disabled=!0))}function ie(){if(!d||!d.recommended_sources)return;let e=d.recommended_sources;if(e.length===0){R.innerHTML=`
...
\u4E3B\u9898\u4ECD\u4F1A\u521B\u5EFA\uFF0C\u4F46\u4E0D\u4F1A\u5173\u8054\u8FD9\u4E9B\u6570\u636E\u6E90\u3002`)}}}let a=[...new Set([...o,...c,...s])];console.log(`[TopicTracker] Source IDs: existing=${o.length}, verified=${c.length}, new=${s.length}, total=${a.length}`),v.innerHTML='<span class="topic-spinner"></span> \u4FDD\u5B58\u4E3B\u9898...';let i={name:e,icon:d.icon,keywords:d.keywords,rss_source_ids:a},r=w?`/api/topics/${w.id}`:"/api/topics",l=await(await fetch(r,{method:w?"PUT":"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(i)})).json();if(l.ok){let h=l.topic?.id||(w?w.id:null);if(!w&&h){let y=a.length;if(y>0){let T=document.getElementById("topicFetchProgress"),g=T?.querySelector(".topic-fetch-progress-text"),f=T?.querySelector(".topic-fetch-progress-fill");T&&(T.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u4E2D...';let k=5;f&&(f.style.width="5%"),g&&(g.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${y} \u4E2A\u6570\u636E\u6E90...`);let S=[{at:20,text:"\u{1F4F1} \u6B63\u5728\u6293\u53D6\u516C\u4F17\u53F7\u6587\u7AE0..."},{at:40,text:"\u{1F4F0} \u6B63\u5728\u83B7\u53D6 RSS \u5185\u5BB9..."},{at:60,text:"\u{1F4BE} \u6B63\u5728\u4FDD\u5B58\u5230\u6570\u636E\u5E93..."},{at:75,text:"\u{1F4CA} \u6B63\u5728\u6574\u7406\u6587\u7AE0\u5217\u8868..."}],B=0,P=setInterval(()=>{if(k<90){k+=(90-k)*.06;let M=Math.round(k);f&&(f.style.width=M+"%"),B<S.length&&M>=S[B].at&&(g&&(g.textContent=S[B].text),B++)}},500);try{let C=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",credentials:"include"})).json();if(clearInterval(P),C.ok){let L=C.fetched||0;f&&(f.style.width="100%"),g&&(g.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${L}\u6761\u65B0\u95FB)`),v.innerHTML='<span class="topic-spinner"></span> \u5904\u7406\u4E2D...',console.log(`[TopicTracker] Fetch completed: ${L} articles from ${y} sources`);let O=C.inactive_sources||[];O.length>0&&await me(O,h)&&(g&&(g.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await fe(h,O,a))}else console.warn("[TopicTracker] Fetch sources warning:",C.error),f&&(f.style.width="100%"),g&&(g.textContent="\u2705 \u6293\u53D6\u5B8C\u6210\uFF0C\u5373\u5C06\u8DF3\u8F6C...");await new Promise(L=>setTimeout(L,800))}catch(M){console.error("[TopicTracker] Fetch sources failed:",M),clearInterval(P)}}X();let x=`topic-${h}`;try{localStorage.setItem("tr_active_tab",x)}catch{}window.location.reload()}else if(h){let y=new Set(w.rss_source_ids||w.rss_sources||[]),x=a.filter(g=>!y.has(g));if(x.length>0){let g=document.getElementById("topicFetchProgress"),f=g?.querySelector(".topic-fetch-progress-text"),k=g?.querySelector(".topic-fetch-progress-fill");g&&(g.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u65B0\u6570\u636E\u6E90...';let S=5;k&&(k.style.width="5%"),f&&(f.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${x.length} \u4E2A\u65B0\u6570\u636E\u6E90...`);let B=setInterval(()=>{S<90&&(S+=(90-S)*.08,k&&(k.style.width=Math.round(S)+"%"))},500);try{let M=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({source_ids:x})})).json();if(clearInterval(B),M.ok){let C=M.fetched||0;k&&(k.style.width="100%"),f&&(f.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${C}\u6761\u65B0\u6587\u7AE0)`);let L=M.inactive_sources||[];L.length>0&&await me(L,h)&&(f&&(f.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await fe(h,L,a))}else k&&(k.style.width="100%"),f&&(f.textContent="\u2705 \u4FDD\u5B58\u5B8C\u6210");await new Promise(C=>setTimeout(C,600))}catch(P){console.error("[TopicTracker] Edit fetch failed:",P),clearInterval(B)}}X();let T=`topic-${h}`;try{localStorage.setItem("tr_active_tab",T)}catch{}window.location.reload()}}else alert(l.error||"\u64CD\u4F5C\u5931\u8D25")}catch(c){console.error("Submit topic failed:",c),alert("\u64CD\u4F5C\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}finally{v.disabled=!1,v.innerHTML=w?"\u4FDD\u5B58\u66F4\u6539":"\u521B\u5EFA\u5E76\u5F00\u59CB\u8FFD\u8E2A"}}function ct(e){ce(e),G(e,!0)}async function st(e){if(!J())return;let t=I.find(o=>o.id===e);if(t){w=t,d={icon:t.icon||"\u{1F3F7}\uFE0F",keywords:[...t.keywords],recommended_sources:[],keywordStats:{}};try{let c=await(await fetch(`/api/topics/${e}/news?limit=1`,{credentials:"include"})).json();if(c.ok&&c.keywords_news)for(let n of t.keywords)d.keywordStats[n]=(c.keywords_news[n]||[]).length}catch(o){console.warn("Failed to load keyword stats:",o)}U.textContent="\u7F16\u8F91\u4E3B\u9898",_.value=t.name,E.style.display="none",v.style.display="",v.textContent="\u4FDD\u5B58\u66F4\u6539",v.disabled=!1,we(),document.getElementById("topicSourcesGroup").style.display="block",await re(t.rss_sources||[]),F.classList.add("active")}}var b={checked:!1,hasAuth:!1,qrPolling:!1,pollTimer:null,sessionId:null};function nt(){$=[];let e=`
        ...
        `;try{let o=await(await fetch("/api/wechat/auth/qr/start",{method:"POST",credentials:"include"})).json();if(!o.ok)throw new Error(o.error||"\u521B\u5EFA\u4F1A\u8BDD\u5931\u8D25");b.sessionId=o.session_id;let c=`/api/wechat/auth/qr/image?t=${Date.now()}`;e.innerHTML=`
            ...
            `,b.qrPolling=!0,te()}catch(t){console.error("[TopicTracker] Start QR error:",t),e.innerHTML=`
            ...
            `}}}async function te(){if(b.qrPolling)try{let t=await(await fetch("/api/wechat/auth/qr/status",{credentials:"include"})).json(),o=document.getElementById("topicWechatQRStatus");if(!o){b.qrPolling=!1;return}if(t.status==="waiting")o.textContent="\u7B49\u5F85\u626B\u7801...",o.className="topic-wechat-qr-status";else if(t.status==="scanned")o.textContent="\u5DF2\u626B\u7801\uFF0C\u8BF7\u5728\u624B\u673A\u4E0A\u786E\u8BA4\u767B\u5F55",o.className="topic-wechat-qr-status scanned";else if(t.status==="confirmed"){o.textContent="\u5DF2\u786E\u8BA4\uFF0C\u6B63\u5728\u5B8C\u6210\u767B\u5F55...",o.className="topic-wechat-qr-status confirmed",b.qrPolling=!1,await pt();return}else if(t.status==="expired"||t.need_refresh){o.textContent="\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F",o.className="topic-wechat-qr-status expired",b.qrPolling=!1;let c=document.getElementById("topicWechatQRArea");c&&(c.innerHTML=`
                    ...
                    `);return}else t.status==="error"&&(o.textContent=t.message||"\u51FA\u9519\u4E86",o.className="topic-wechat-qr-status error");b.qrPolling&&(b.pollTimer=setTimeout(te,2e3))}catch(e){console.error("[TopicTracker] Poll QR error:",e),b.qrPolling&&(b.pollTimer=setTimeout(te,3e3))}}async function pt(){let e=document.getElementById("topicWechatQRArea");try{let o=await(await fetch("/api/wechat/auth/qr/complete-and-share",{method:"POST",credentials:"include"})).json();if(!o.ok)throw new Error(o.error||"\u767B\u5F55\u5931\u8D25");e&&(e.innerHTML=`
                ...
                `),setTimeout(()=>{b.hasAuth=!0;let c=document.getElementById("topicWechatAuthArea"),n=document.getElementById("topicWechatSearchArea");c&&(c.style.display="none"),n&&(n.style.display="")},1e3)}catch(t){console.error("[TopicTracker] Complete QR error:",t),e&&(e.innerHTML=`
                    ...
                    <button class="topic-wechat-qr-refresh" onclick="TopicTracker.refreshWechatQR()">\u91CD\u8BD5</button>
                `)}}function le(){b.qrPolling=!1,b.pollTimer&&(clearTimeout(b.pollTimer),b.pollTimer=null)}function ut(){le(),Se()}async function mt(){if(document.querySelector(".topic-manual-tab.active").dataset.tab==="rss"){let t=document.getElementById("manualRssName").value.trim(),o=document.getElementById("manualRssUrl").value.trim();if(!o){alert("\u8BF7\u8F93\u5165 RSS URL");return}d.recommended_sources||(d.recommended_sources=[]),d.recommended_sources.push({type:"rss",name:t||o,url:o})}else{if($.length===0){alert("\u8BF7\u9009\u62E9\u81F3\u5C11\u4E00\u4E2A\u516C\u4F17\u53F7");return}d.recommended_sources||(d.recommended_sources=[]);for(let t of $)d.recommended_sources.push({type:"wechat_mp",name:t.nickname,wechat_id:t.fakeid});$=[]}$e(),w&&w.rss_sources?await re(w.rss_sources):ie(),document.getElementById("topicSourcesGroup").style.display="block"}async function ft(){let e=_.value.trim();if(!e){alert("\u8BF7\u5148\u8F93\u5165\u4E3B\u9898\u540D\u79F0");return}let t=document.querySelector(".topic-sources-add .topic-ai-btn"),o=["\u{1F50D} \u6B63\u5728\u641C\u7D22\u6570\u636E\u6E90...","\u{1F4E1} \u6B63\u5728\u9A8C\u8BC1 RSS \u6E90...","\u2705 \u6B63\u5728\u9A8C\u8BC1\u516C\u4F17\u53F7..."],c=0,n=()=>{t&&(t.innerHTML=`<span class="topic-spinner"></span> ${o[c]}`)};t&&(t.disabled=!0,n());let s=setInterval(()=>{c=Math.min(c+1,o.length-1),n()},2e3);try{let i=await(await fetch("/api/topics/generate-keywords",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({topic_name:e})})).json();if(i.ok&&i.recommended_sources&&i.recommended_sources.length>0){let r=d.recommended_sources||[],p=new Set(r.map(l=>l.type==="rss"?l.url:l.wechat_id)),u=0;for(let l of i.recommended_sources){let h=l.type==="rss"?l.url:l.wechat_id;p.has(h)||(r.push(l),p.add(h),u++)}d.recommended_sources=r,document.getElementById("topicSourcesGroup").style.display="block",w?await re(w.rss_sources||[]):ie(),u>0?console.log(`[TopicTracker] \u65B0\u589E ${u} \u4E2A\u63A8\u8350\u6570\u636E\u6E90`):alert("AI \u63A8\u8350\u7684\u6570\u636E\u6E90\u5DF2\u5168\u90E8\u6DFB\u52A0\u8FC7")}else alert("\u672A\u627E\u5230\u65B0\u7684\u63A8\u8350\u6570\u636E\u6E90")}catch(a){console.error("Regenerate sources failed:",a),alert("\u641C\u7D22\u6570\u636E\u6E90\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}finally{clearInterval(s),t&&(t.disabled=!1,t.innerHTML="\u{1F504} AI \u65B0\u589E\u6570\u636E\u6E90")}}async function ht(e){let t=I.find(c=>c.id===e);if(!(!t||!await Me(`\u786E\u5B9A\u8981\u5220\u9664\u4E3B\u9898\u300C${t.name}\u300D\u5417\uFF1F\u5220\u9664\u540E\u4E0D\u53EF\u6062\u590D\u3002`)))try{let n=await(await fetch(`/api/topics/${e}`,{method:"DELETE",credentials:"include"})).json();if(n.ok){let s="topic-"+e,a=document.querySelector(`.sub-tab[data-category="${s}"]`);a&&(a.remove(),console.log(`[TopicTracker] Removed tab for topic ${e}`));let i=document.getElementById(`tab-${s}`);i&&(i.remove(),console.log(`[TopicTracker] Removed pane for topic ${e}`)),ce(e);try{localStorage.getItem("tr_active_tab")===s&&(localStorage.removeItem("tr_active_tab"),console.log(`[TopicTracker] Cleared saved tab for deleted topic ${e}`))}catch{}A.switchTab("my-tags"),I=I.filter(r=>r.id!==e),K.toast?.show&&K.toast.show(`\u5DF2\u5220\u9664\u4E3B\u9898\u300C${t.name}\u300D`,{variant:"success"})}else alert(n.error||"\u5220\u9664\u5931\u8D25")}catch(c){console.error("Delete topic failed:",c),alert("\u5220\u9664\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}}function Ie(e){if(!e)return"";let t=new Date(e*1e3),c=new Date-t,n=Math.floor(c/6e4),s=Math.floor(c/36e5),a=Math.floor(c/864e5);if(n<1)return"\u521A\u521A";if(n<60)return n+"\u5206\u949F\u524D";if(s<24)return s+"\u5C0F\u65F6\u524D";if(a<7)return a+"\u5929\u524D";let i=t.getMonth()+1,r=t.getDate();return i+"-"+r}window.TopicTracker={init:se,openModal:ze,closeModal:X,generateKeywords:be,addKeyword:Xe,removeKeyword:Ze,selectAllSources:et,deselectAllSources:tt,submitTopic:ot,refreshTopic:ct,editTopic:st,deleteTopic:ht,loadTopics:Re,regenerateSources:ft,showManualAddForm:nt,hideManualAddForm:$e,switchManualTab:lt,addManualSource:mt,searchWechatMps:ke,doMpSearch:at,toggleMpSelection:it,removeMpSelection:rt,refreshWechatQR:ut,retryLoadTopic:Qe,loadTopicNews:G,closeQrcodeModal:ne,retryAfterAuth:Ve,refreshQrcodeModal:Ye};de(se);
