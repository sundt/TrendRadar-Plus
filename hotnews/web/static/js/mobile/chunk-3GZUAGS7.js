import{a as f}from"./chunk-YRLDCOZY.js";var c=null,a=null,u=null,b=null,p=0,y=5*60;async function w(){return c||new Promise((t,i)=>{if(window.QRCode){c=window.QRCode,t(c);return}let s=document.createElement("script");s.src="/static/js/lib/qrcode.min.js",s.onload=()=>{c=window.QRCode,t(c)},s.onerror=i,document.head.appendChild(s)})}async function Q(){try{let t=await fetch("/api/subscription/status",{credentials:"include"});return t.ok?await t.json():null}catch(t){return console.error("Get subscription status error:",t),null}}async function S(){if(!f.getUser()){window.Toast?.show("\u8BF7\u5148\u767B\u5F55","error"),window.openLoginModal?.();return}let i=document.getElementById("subscriptionModal");i||(i=k(),document.body.appendChild(i)),a=null,e(),i.classList.add("open"),await x()}function C(){let t=document.getElementById("subscriptionModal");t&&t.classList.remove("open"),e(),l(),a=null}function k(){let t=document.createElement("div");return t.id="subscriptionModal",t.className="subscription-modal",t.innerHTML=`
    ...
    `,q(),t}async function x(){let t=document.getElementById("subscriptionPlansSection"),i=document.getElementById("subscriptionStatusHint");try{let[s,o]=await Promise.all([fetch("/api/subscription/plans"),fetch("/api/subscription/status",{credentials:"include"})]),n=await s.json(),r=await o.json();if(i)if(r.is_vip){let m=new Date(r.expire_at*1e3);i.innerHTML=`<span class="vip-badge">VIP</span> \u5230\u671F: ${m.toLocaleDateString("zh-CN")} \xB7 \u5269\u4F59 ${r.usage_remaining} \u4E2A\u989D\u5EA6`}else i.textContent="\u5F00\u901A\u4F1A\u5458\uFF0C\u89E3\u9501\u4E13\u5C5E\u8FFD\u8E2A\u4E3B\u9898";let d=n.plans||[];if(d.length===0){t.innerHTML='<div class="subscription-loading">\u6682\u65E0\u53EF\u7528\u5957\u9910</div>';return}t.innerHTML=`
        ...
        `}}function E(t,i=!1){let s=t.badge?`<div class="subscription-plan-badge">${t.badge}</div>`:"";return`
    ...
    `,document.head.appendChild(t)}window.openSubscriptionModal=S;window.closeSubscriptionModal=C;window.selectSubscriptionPlan=I;window.showSubscriptionPlans=P;window.manualCheckSubscription=R;window.loadSubscriptionPlans=x;export{Q as a,S as b,C as c};
