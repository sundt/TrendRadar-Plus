/**
 * OpenClaw Timeline Module - Timeline Card Mode
 * 所有 OpenClaw 相关文章按时间倒序排列，每卡片50条，横向滚动无限加载。
 */
...
function _buildNewsItemsHtml(items) {
    ...
    if (!arr.length) return '<li class="tr-openclaw-empty" aria-hidden="true">暂无内容</li>';
    return arr.map((n, idx) => {
        const stableId = escapeHtml(n?.stable_id || '');
        const title = escapeHtml(n?.display_title || n?.title || '');
        const url = escapeHtml(n?.url || '#');
        const sourceName = escapeHtml(n?.source_name || 'OpenClaw');
        ...
        const sourceId = escapeHtml(n?.source_id || 'openclaw');
...
async function _fetchTimelineBatch(limit, offset) {
    const url = `/api/rss/openclaw/timeline?limit=${limit}&offset=${offset}`;
...
function _appendCard(items, cardIndex, container) {
    ...
    card.className = 'platform-card tr-openclaw-card';
...
window.HotNews.openclawTimeline = {
...
export const openclawTimeline = { resetState };
