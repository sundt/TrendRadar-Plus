...
`),t=t.replace(/<p>\s*<\/p>/g,""),t=t.replace(/<p><hr><\/p>/g,"<hr>"),t=t.replace(/<p>(<table>)/g,"$1"),t=t.replace(/(<\/table>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/<p>(<blockquote>)/g,"$1"),t=t.replace(/(<\/blockquote>)<\/p>/g,"$1"),t=t.replace(/(<h[1-3]>(?:✅\s*|📋\s*)?行动清单\s*<\/h[1-3]>)/gi,'<div class="action-list-header">$1<button class="action-list-add-btn" onclick="addActionListToTodo()">+ Todo</button></div>'),t}function Qe(){if(document.getElementById("summaryModal"))return;document.body.insertAdjacentHTML("beforeend",`
    ...
    `,m.style.display="none",j(),ie=setTimeout(()=>{let l=document.getElementById("summarySlowHint");l&&(l.style.display="block")},Vt),de=setTimeout(()=>{Zt(o,n,s),d.innerHTML=`
        ...
        `},Xt);try{let l=await fetch(`/api/summary/failures/check?url=${encodeURIComponent(o)}`);if(l.ok){let v=await l.json();if(v.summarizable)v.warning&&console.log("[Summary] Warning:",v.warning);else{j(),d.innerHTML=`
                    <div class="summary-access-error">
...
`);for(let J of g)if(J.startsWith("data: "))try{let u=JSON.parse(J.slice(6));switch(u.type){case"status":let Y=document.getElementById("summaryStatusText");Y&&(Y.textContent=u.message);break;case"type":S=u.article_type,_=u.article_type_name,window._currentTypeConfidence=u.confidence||0;break;case"chunk":P||(P=!0,j(),d.innerHTML=`
                            ...
                            `,le(o,S,_,!0,$,O,ft),ce(e,!0),u.tags&&window.ArticleTags){console.log("[Summary] Applying cached tags for newsId:",e,"tags:",u.tags);let w=document.querySelector(`.news-item[data-news-id="${e}"]`);w||(w=document.querySelector(`.news-item[data-url="${o}"]`)),w&&(console.log("[Summary] Found news item for cached, applying tags"),window.ArticleTags.applyTags(w,u.tags),w.dataset.tagsLoaded="true")}return;case"done":$=u.token_usage||null;let pt=u.token_balance!==void 0?{token_balance:u.token_balance,tokens_used:u.tokens_used,default_tokens:1e5}:null,ye=document.getElementById("summaryStreamContent");if(ye){ye.classList.remove("summary-streaming");let w=Le(b);ye.innerHTML=q(w)}let gt=window._currentTypeConfidence||0;if(le(o,S,_,!1,$,null,pt,gt),ce(e,!0),u.tags&&window.ArticleTags){console.log("[Summary] Applying tags for newsId:",e,"tags:",u.tags);let w=document.querySelector(`.news-item[data-news-id="${e}"]`);w||(w=document.querySelector(`.news-item[data-url="${o}"]`)),w?(console.log("[Summary] Found news item, applying tags"),window.ArticleTags.applyTags(w,u.tags),w.dataset.tagsLoaded="true"):console.log("[Summary] News item not found in DOM")}break;case"short_content":j(),d.innerHTML=`
            ...
            `;else{Ve();return}}}function le(e,t,o,n,s,r=null,a=null,i=null){let d=document.getElementById("summaryModalFooter"),m=eo[t]||"\u{1F4DD}",l="",c=s?.total_tokens||0;c>0&&(l=`<span class="summary-token-tag" title="\u672C\u6B21\u6D88\u8017">\u{1FA99} ${to(c)}</span>`);let f=r==="up"?"active":"",y=r==="down"?"active":"";d.innerHTML=`
...
`);for(let fe of x)if(fe.startsWith("data: "))try{let g=JSON.parse(fe.slice(6));switch(g.type){case"status":let J=document.getElementById("summaryStatusText");J&&(J.textContent=g.message);break;case"type":y=g.article_type,b=g.article_type_name;break;case"chunk":S||(S=!0,i.innerHTML=`
    ...
    `;t.innerHTML=o}function Ye(){let e=document.getElementById("favoritesPanelBody");if(!e)return;let t=me();t.length>0?e.innerHTML=`
        ...
        `}async function lt(){let e=document.getElementById("favoritesPanelBody");if(!e)return;if(e.innerHTML='<div class="favorites-loading">\u52A0\u8F7D\u4E2D...</div>',!T.getUser()){Ye();return}let o=await yo();if(o.needsAuth){Ye();return}if(o.error){e.innerHTML=`
            <div class="favorites-empty">
