...
`),t=t.replace(/<p>\s*<\/p>/g,""),t=t.replace(/<p><hr><\/p>/g,"<hr>"),t=t.replace(/<p>(<table>)/g,"$1"),t=t.replace(/(<\/table>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/<p>(<blockquote>)/g,"$1"),t=t.replace(/(<\/blockquote>)<\/p>/g,"$1"),t=t.replace(/(<h[1-3]>(?:✅\s*|📋\s*)?行动清单\s*<\/h[1-3]>)/gi,'<div class="action-list-header">$1<button class="action-list-add-btn" onclick="addActionListToTodo()">+ Todo</button></div>'),t}function it(){if(document.getElementById("summaryModal"))return;document.body.insertAdjacentHTML("beforeend",`
    ...
    `)}async function He(e,t,o,n,s){if(!T.getUser()){x();return}it();let r=document.getElementById("summaryModal"),d=document.getElementById("summaryModalTitle"),l=document.getElementById("summaryModalBody"),m=document.getElementById("summaryModalFooter");if(y=e,_=t,W=o,ot=n,nt=s,ge=!0,window._currentSummaryNewsId=e,window._currentSummaryNewsTitle=t,window._currentSummaryNewsUrl=o,tt(),d){let i=t&&t.length>50?t.substring(0,50)+"...":t||"AI \u603B\u7ED3";d.textContent=`\u2728 ${i}`}r.classList.add("open"),document.body.style.overflow="hidden",l.innerHTML=`
    ...
    `,m.style.display="none",I(),Z=setTimeout(()=>{console.log("[Summary] 5s timeout, starting countdown");let i=document.getElementById("summarySlowHint"),c=document.getElementById("summaryCountdownText");i&&(i.style.display="block"),q=5;let p=()=>{c&&(c.textContent=`\u52A0\u8F7D\u8F83\u6162\uFF0C${q} \u79D2\u540E\u4E3A\u60A8\u6253\u5F00\u539F\u6587...`)};p(),te=setInterval(()=>{q--,q>0?p():ne()},1e3),oe=setTimeout(()=>{console.log("[Summary] 10s timeout, showing click hint"),ne();let f=document.getElementById("summarySlowHint");f&&(f.innerHTML='\u52A0\u8F7D\u8F83\u6162\uFF0C\u8BF7 <a href="'+o+'" target="_blank" rel="noopener noreferrer" style="color: #3b82f6; text-decoration: underline;">\u70B9\u51FB\u9605\u8BFB\u539F\u6587</a>')},st-ve)},ve),ee=setTimeout(()=>{rt(o,n,s),I();let i=document.getElementById("summaryModal");i&&(i.classList.remove("open"),document.body.style.overflow=""),ge=!1,y=null,o&&window.open(o,"_blank","noopener,noreferrer")},at);try{let i=await fetch(`/api/summary/failures/check?url=${encodeURIComponent(o)}`);if(i.ok){let b=await i.json();if(console.log("[Summary] Check result:",b),b.summarizable)b.warning&&console.log("[Summary] Warning:",b.warning);else{console.log("[Summary] URL blocked, showing blocked UI:",o),I();let k=!xe()&&io(),se=k?lo():null;!xe()&&!k?l.innerHTML=`
                    ...
                    `,m.style.display="none";return}}let c=await fetch("/api/summary/stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:o,title:t,news_id:e,source_id:n,source_name:s})});if(!c.ok){let b=await c.json();throw new Error(b.detail||"\u751F\u6210\u5931\u8D25")}let p=c.body.getReader(),f=new TextDecoder,h="",S="other",C="\u5176\u4ED6",D=!1,F=null;for(;;){let{done:b,value:k}=await p.read();if(b)break;let g=f.decode(k,{stream:!0}).split(`
`);for(let Q of g)if(Q.startsWith("data: "))try{let u=JSON.parse(Q.slice(6));switch(u.type){case"status":let ae=document.getElementById("summaryStatusText");ae&&(ae.textContent=u.message);break;case"type":S=u.article_type,C=u.article_type_name,window._currentTypeConfidence=u.confidence||0;break;case"chunk":D||(D=!0,I(),l.innerHTML=`
                                ...
                                `),h+=u.content;let N=document.getElementById("summaryStreamContent");if(N){let v=h;v=v.replace(/\[TAGS_?START\][\s\S]*$/gi,""),N.innerHTML=O(v)+'<span class="summary-cursor">\u258C</span>',N.scrollTop=N.scrollHeight}break;case"cached":I(),h=u.summary,S=u.article_type,C=u.article_type_name,F=u.token_usage||null;let M=u.feedback||null,Lt=u.token_balance!==void 0?{token_balance:u.token_balance,tokens_used:u.tokens_used,default_tokens:1e5}:null,It=Ae(h);if(l.innerHTML=`
                                <div class="summary-content">
                                    ${O(It)}
                                </div>
                            `,fe(o,S,C,!0,F,M,Lt),ye(e,!0),u.tags&&window.ArticleTags){console.log("[Summary] Applying cached tags for newsId:",e,"tags:",u.tags);let v=document.querySelector(`.news-item[data-news-id="${e}"]`);v||(v=document.querySelector(`.news-item[data-url="${o}"]`)),v&&(console.log("[Summary] Found news item for cached, applying tags"),window.ArticleTags.applyTags(v,u.tags),v.dataset.tagsLoaded="true")}return;case"done":F=u.token_usage||null;let _t=u.token_balance!==void 0?{token_balance:u.token_balance,tokens_used:u.tokens_used,default_tokens:1e5}:null,be=document.getElementById("summaryStreamContent");if(be){be.classList.remove("summary-streaming");let v=Ae(h);be.innerHTML=O(v)}let Bt=window._currentTypeConfidence||0;if(fe(o,S,C,!1,F,null,_t,Bt),ye(e,!0),u.tags&&window.ArticleTags){console.log("[Summary] Applying tags for newsId:",e,"tags:",u.tags);let v=document.querySelector(`.news-item[data-news-id="${e}"]`);v||(v=document.querySelector(`.news-item[data-url="${o}"]`)),v?(console.log("[Summary] Found news item, applying tags"),window.ArticleTags.applyTags(v,u.tags),v.dataset.tagsLoaded="true"):console.log("[Summary] News item not found in DOM")}break;case"short_content":I(),l.innerHTML=`
            ...
            `;else{Pe();return}}}function fe(e,t,o,n,s,a=null,r=null,d=null){let l=document.getElementById("summaryModalFooter"),m=mo[t]||"\u{1F4DD}",i="",c=s?.total_tokens||0;c>0&&(i=`<span class="summary-token-tag" title="\u672C\u6B21\u6D88\u8017">\u{1FA99} ${po(c)}</span>`);let p=a==="up"?"active":"",f=a==="down"?"active":"";l.innerHTML=`
