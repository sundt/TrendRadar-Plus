import{a as J,b as R,d as H}from"./chunk-SNAK7DZO.js";import{a as N}from"./chunk-YRL7WKAS.js";import"./chunk-2LJVOGXX.js";import{a as K,b as de,c as m}from"./chunk-I35K533E.js";function Ie(e){return new Promise(t=>{let o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:10010;display:flex;align-items:center;justify-content:center;";let c=document.createElement("div");c.style.cssText="background:#fff;border-radius:12px;padding:24px;max-width:320px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.2);text-align:center;",c.innerHTML=`
            ...
            </div>`,o.appendChild(c),document.body.appendChild(o);let n=s=>{o.remove(),t(s)};c.querySelector(".confirm-cancel").onclick=()=>n(!1),c.querySelector(".confirm-ok").onclick=()=>n(!0),o.addEventListener("click",s=>{s.target===o&&n(!1)})})}var M=[],w=null,d=null,Q=new Map,W=0;function te(e){return Q.has(e)||Q.set(e,{loading:!1,loaded:!1}),Q.get(e)}function oe(e){Q.set(e,{loading:!1,loaded:!1})}function he(){Q.clear()}var F=null,D=null,_=null,ge=null,B=null,E=null,v=null,Y=null;function pe(){let t=N.getUser?.()?.id;document.querySelectorAll(".category-tab.topic-tab").forEach(c=>{let n=c.dataset.ownerUserId,s=c.dataset.category;if(!t||n&&String(n)!==String(t)){console.warn(`[TopicTracker] Removing topic tab: ${s}, owner=${n}, current=${t}`),c.remove();let a=document.getElementById(`tab-${s}`);a&&a.remove()}})}function qe(){document.querySelectorAll(".category-tab.topic-tab[data-owner-user-id]").forEach(t=>{let o=t.dataset.category;console.log(`[TopicTracker] Removing server-rendered topic tab: ${o}`),t.remove();let c=document.getElementById(`tab-${o}`);c&&c.remove()})}function ce(){if(console.log("[TopicTracker] Initializing..."),!document.body){console.warn("[TopicTracker] document.body not ready, retrying..."),setTimeout(ce,100);return}qe(),pe(),Be(),z(),U(),J.on("tab:switched",t=>{let o=t?.categoryId;if(console.log("[TopicTracker] tab:switched event received, categoryId:",o),o&&String(o).startsWith("topic-")){let c=String(o).replace("topic-","");console.log("[TopicTracker] Topic tab switched, loading:",c),P(c)}}),J.on("viewer:rendered",()=>{console.log("[TopicTracker] viewer:rendered event received, resetting states and setting up listeners..."),W++,console.log("[TopicTracker] Generation bumped to:",W),pe(),he(),U(),z();let t=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(t){let o=t.id.replace("tab-topic-","");console.log("[TopicTracker] Active topic tab detected after re-render:",o),setTimeout(()=>P(o),100)}}),window.addEventListener("authStateChanged",t=>{console.log("[TopicTracker] Auth state changed, reloading topics..."),Ee(),t?.detail?.user&&z()});let e=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(e){let t=e.id.replace("tab-topic-","");console.log("[TopicTracker] Topic tab already active on page load:",t),setTimeout(()=>P(t),200)}}function Ee(){document.querySelectorAll(".category-tab.topic-tab").forEach(e=>e.remove()),document.querySelectorAll('.tab-pane[id^="tab-topic-"]').forEach(e=>e.remove()),he(),M=[]}async function z(){try{N.initialized||(console.log("[TopicTracker] Waiting for authState to initialize..."),await N.init())}catch(o){console.warn("[TopicTracker] authState init failed:",o)}if(!se()){console.log("[TopicTracker] User not logged in, skipping topic tabs loading");return}let e=document.querySelector(".category-tabs");if(!e){console.warn("[TopicTracker] category-tabs not found");return}let t=Ce(e);try{console.log("[TopicTracker] Loading topics from API...");let c=await(await fetch("/api/topics",{credentials:"include"})).json();t&&t.remove(),c.ok&&c.topics?.length>0?(M=c.topics,console.log(`[TopicTracker] Loaded ${M.length} topics, rendering tabs...`),Le(M),_e()):(console.log("[TopicTracker] No topics found or API error:",c.detail||c.error),M=[])}catch(o){console.error("[TopicTracker] Failed to load topics:",o),t&&t.remove(),M=[]}}function _e(){try{let e=localStorage.getItem("hotnews_active_tab"),t=localStorage.getItem("tr_active_tab"),o=e&&e.startsWith("topic-")?e:t;if(o&&o.startsWith("topic-")){let c=document.querySelector(`.category-tab[data-category="${o}"]`),n=document.getElementById(`tab-${o}`);if(c&&n)console.log(`[TopicTracker] Restoring to topic tab: ${o}`),H.switchTab(o);else{console.log(`[TopicTracker] Topic tab not found, clearing saved tab: ${o}`),localStorage.removeItem("tr_active_tab");try{R.consumeNavigationState?.()}catch{}H.switchTab("my-tags")}}}catch(e){console.error("[TopicTracker] Failed to restore topic tab:",e)}}function Ce(e){let t=e.querySelector('[data-category="my-tags"]'),o=document.createElement("div");o.className="topic-tabs-loading",o.id="topicTabsSkeleton";for(let c=0;c<2;c++){let n=document.createElement("div");n.className="topic-tab-skeleton",n.innerHTML=`
        ...
        `}function se(){return!!N.getUser()}function ve(){typeof window.openLoginModal=="function"?window.openLoginModal():alert("\u8BF7\u5148\u767B\u5F55")}function Oe(){if(!se()){ve();return}w=null,d=null,D.textContent="\u65B0\u5EFA\u8FFD\u8E2A\u4E3B\u9898",_.value="",document.getElementById("topicIconGroup").style.display="none",document.getElementById("topicKeywordsGroup").style.display="none",document.getElementById("topicSourcesGroup").style.display="none",E.style.display="",E.disabled=!1,E.innerHTML="\u{1F916} \u751F\u6210\u5173\u952E\u8BCD\u548C\u63A8\u8350\u6E90",v.style.display="none",F.classList.add("active"),_.focus()}function V(){F.classList.remove("active"),w=null,d=null}function ue(e,t){return new Promise(o=>{let c=`
        ...
        `;try{let o=await(await fetch("/api/wechat/auth/qr/start",{method:"POST",credentials:"include"})).json();if(!o.ok)throw new Error(o.error||"\u521B\u5EFA\u4F1A\u8BDD\u5931\u8D25");q.sessionId=o.session_id;let c=`/api/wechat/auth/qr/image?t=${Date.now()}`;e.innerHTML=`
            ...
            `}).join("")+`
            <button class="topic-keyword-add" onclick="TopicTracker.addKeyword()">+ \u6DFB\u52A0\u5173\u952E\u8BCD</button>
        `;ge.innerHTML=t}function Ye(){let e=prompt("\u8BF7\u8F93\u5165\u5173\u952E\u8BCD\uFF1A");!e||!e.trim()||(d||(d={icon:"\u{1F3F7}\uFE0F",keywords:[],recommended_sources:[]},document.getElementById("topicIconGroup").style.display="block",document.getElementById("topicKeywordsGroup").style.display="block"),d.keywords.includes(e.trim())||(d.keywords.push(e.trim()),ae(),v.disabled=!1))}function Ve(e){d&&(d.keywords=d.keywords.filter(t=>t!==e),ae(),d.keywords.length===0&&(v.disabled=!0))}function ie(){if(!d||!d.recommended_sources)return;let e=d.recommended_sources;if(e.length===0){B.innerHTML=`
...
\u4E3B\u9898\u4ECD\u4F1A\u521B\u5EFA\uFF0C\u4F46\u4E0D\u4F1A\u5173\u8054\u8FD9\u4E9B\u6570\u636E\u6E90\u3002`)}}}let a=[...new Set([...o,...c,...s])];console.log(`[TopicTracker] Source IDs: existing=${o.length}, verified=${c.length}, new=${s.length}, total=${a.length}`),v.innerHTML='<span class="topic-spinner"></span> \u4FDD\u5B58\u4E3B\u9898...';let i={name:e,icon:d.icon,keywords:d.keywords,rss_source_ids:a},r=w?`/api/topics/${w.id}`:"/api/topics",l=await(await fetch(r,{method:w?"PUT":"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(i)})).json();if(l.ok){let h=l.topic?.id||(w?w.id:null);if(!w&&h){let y=a.length;if(y>0){let T=document.getElementById("topicFetchProgress"),g=T?.querySelector(".topic-fetch-progress-text"),f=T?.querySelector(".topic-fetch-progress-fill");T&&(T.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u4E2D...';let k=5;f&&(f.style.width="5%"),g&&(g.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${y} \u4E2A\u6570\u636E\u6E90...`);let S=[{at:20,text:"\u{1F4F1} \u6B63\u5728\u6293\u53D6\u516C\u4F17\u53F7\u6587\u7AE0..."},{at:40,text:"\u{1F4F0} \u6B63\u5728\u83B7\u53D6 RSS \u5185\u5BB9..."},{at:60,text:"\u{1F4BE} \u6B63\u5728\u4FDD\u5B58\u5230\u6570\u636E\u5E93..."},{at:75,text:"\u{1F4CA} \u6B63\u5728\u6574\u7406\u6587\u7AE0\u5217\u8868..."}],A=0,j=setInterval(()=>{if(k<90){k+=(90-k)*.06;let I=Math.round(k);f&&(f.style.width=I+"%"),A<S.length&&I>=S[A].at&&(g&&(g.textContent=S[A].text),A++)}},500);try{let C=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",credentials:"include"})).json();if(clearInterval(j),C.ok){let L=C.fetched||0;f&&(f.style.width="100%"),g&&(g.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${L}\u6761\u65B0\u95FB)`),v.innerHTML='<span class="topic-spinner"></span> \u5904\u7406\u4E2D...',console.log(`[TopicTracker] Fetch completed: ${L} articles from ${y} sources`);let O=C.inactive_sources||[];O.length>0&&await ue(O,h)&&(g&&(g.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await me(h,O,a))}else console.warn("[TopicTracker] Fetch sources warning:",C.error),f&&(f.style.width="100%"),g&&(g.textContent="\u2705 \u6293\u53D6\u5B8C\u6210\uFF0C\u5373\u5C06\u8DF3\u8F6C...");await new Promise(L=>setTimeout(L,800))}catch(I){console.error("[TopicTracker] Fetch sources failed:",I),clearInterval(j)}}V();let x=`topic-${h}`;try{localStorage.setItem("tr_active_tab",x)}catch{}window.location.reload()}else if(h){let y=new Set(w.rss_source_ids||w.rss_sources||[]),x=a.filter(g=>!y.has(g));if(x.length>0){let g=document.getElementById("topicFetchProgress"),f=g?.querySelector(".topic-fetch-progress-text"),k=g?.querySelector(".topic-fetch-progress-fill");g&&(g.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u65B0\u6570\u636E\u6E90...';let S=5;k&&(k.style.width="5%"),f&&(f.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${x.length} \u4E2A\u65B0\u6570\u636E\u6E90...`);let A=setInterval(()=>{S<90&&(S+=(90-S)*.08,k&&(k.style.width=Math.round(S)+"%"))},500);try{let I=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({source_ids:x})})).json();if(clearInterval(A),I.ok){let C=I.fetched||0;k&&(k.style.width="100%"),f&&(f.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${C}\u6761\u65B0\u6587\u7AE0)`);let L=I.inactive_sources||[];L.length>0&&await ue(L,h)&&(f&&(f.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await me(h,L,a))}else k&&(k.style.width="100%"),f&&(f.textContent="\u2705 \u4FDD\u5B58\u5B8C\u6210");await new Promise(C=>setTimeout(C,600))}catch(j){console.error("[TopicTracker] Edit fetch failed:",j),clearInterval(A)}}V();let T=`topic-${h}`;try{localStorage.setItem("tr_active_tab",T)}catch{}window.location.reload()}}else alert(l.error||"\u64CD\u4F5C\u5931\u8D25")}catch(c){console.error("Submit topic failed:",c),alert("\u64CD\u4F5C\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}finally{v.disabled=!1,v.innerHTML=w?"\u4FDD\u5B58\u66F4\u6539":"\u521B\u5EFA\u5E76\u5F00\u59CB\u8FFD\u8E2A"}}function tt(e){oe(e),G(e,!0)}async function ot(e){if(!se()){ve();return}let t=M.find(o=>o.id===e);if(t){w=t,d={icon:t.icon||"\u{1F3F7}\uFE0F",keywords:[...t.keywords],recommended_sources:[],keywordStats:{}};try{let c=await(await fetch(`/api/topics/${e}/news?limit=1`,{credentials:"include"})).json();if(c.ok&&c.keywords_news)for(let n of t.keywords)d.keywordStats[n]=(c.keywords_news[n]||[]).length}catch(o){console.warn("Failed to load keyword stats:",o)}D.textContent="\u7F16\u8F91\u4E3B\u9898",_.value=t.name,E.style.display="none",v.style.display="",v.textContent="\u4FDD\u5B58\u66F4\u6539",v.disabled=!1,we(),document.getElementById("topicSourcesGroup").style.display="block",await re(t.rss_sources||[]),F.classList.add("active")}}var b={checked:!1,hasAuth:!1,qrPolling:!1,pollTimer:null,sessionId:null};function ct(){$=[];let e=`
