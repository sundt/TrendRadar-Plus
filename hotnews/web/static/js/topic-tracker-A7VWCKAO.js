import{a as K,b as R,d as H}from"./chunk-SNAK7DZO.js";import{a as J}from"./chunk-YRL7WKAS.js";import"./chunk-2LJVOGXX.js";import{a as G,b as le,c as m}from"./chunk-I35K533E.js";function Ee(e){return new Promise(t=>{let o=document.createElement("div");o.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:10010;display:flex;align-items:center;justify-content:center;";let c=document.createElement("div");c.style.cssText="background:#fff;border-radius:12px;padding:24px;max-width:320px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.2);text-align:center;",c.innerHTML=`
            ...
            </div>`,o.appendChild(c),document.body.appendChild(o);let n=s=>{o.remove(),t(s)};c.querySelector(".confirm-cancel").onclick=()=>n(!1),c.querySelector(".confirm-ok").onclick=()=>n(!0),o.addEventListener("click",s=>{s.target===o&&n(!1)})})}var x=[],w=null,d=null,N=new Map,Q=0;function te(e){return N.has(e)||N.set(e,{loading:!1,loaded:!1}),N.get(e)}function oe(e){N.set(e,{loading:!1,loaded:!1})}function fe(){N.clear()}var W=null,D=null,_=null,he=null,B=null,E=null,v=null,Y=null;function de(){let t=J.getUser?.()?.id;document.querySelectorAll(".category-tab.topic-tab").forEach(c=>{let n=c.dataset.ownerUserId,s=c.dataset.category;if(!t||n&&String(n)!==String(t)){console.warn(`[TopicTracker] Removing topic tab: ${s}, owner=${n}, current=${t}`),c.remove();let a=document.getElementById(`tab-${s}`);a&&a.remove()}})}function _e(){document.querySelectorAll(".category-tab.topic-tab[data-owner-user-id]").forEach(t=>{let o=t.dataset.category;console.log(`[TopicTracker] Removing server-rendered topic tab: ${o}`),t.remove();let c=document.getElementById(`tab-${o}`);c&&c.remove()})}function ce(){if(console.log("[TopicTracker] Initializing..."),!document.body){console.warn("[TopicTracker] document.body not ready, retrying..."),setTimeout(ce,100);return}_e(),de(),Re(),O(),z(),K.on("tab:switched",t=>{let o=t?.categoryId;if(console.log("[TopicTracker] tab:switched event received, categoryId:",o),o&&String(o).startsWith("topic-")){let c=String(o).replace("topic-","");console.log("[TopicTracker] Topic tab switched, loading:",c),P(c)}}),K.on("viewer:rendered",()=>{console.log("[TopicTracker] viewer:rendered event received, resetting states and setting up listeners..."),Q++,console.log("[TopicTracker] Generation bumped to:",Q),de(),fe(),z(),O();let t=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(t){let o=t.id.replace("tab-topic-","");console.log("[TopicTracker] Active topic tab detected after re-render:",o),setTimeout(()=>P(o),100)}}),window.addEventListener("authStateChanged",t=>{console.log("[TopicTracker] Auth state changed, reloading topics..."),Ce(),t?.detail?.user&&O()});let e=document.querySelector('.tab-pane.active[id^="tab-topic-"]');if(e){let t=e.id.replace("tab-topic-","");console.log("[TopicTracker] Topic tab already active on page load:",t),setTimeout(()=>P(t),200)}}function Ce(){document.querySelectorAll(".category-tab.topic-tab").forEach(e=>e.remove()),document.querySelectorAll('.tab-pane[id^="tab-topic-"]').forEach(e=>e.remove()),fe(),x=[]}async function O(){if(!document.cookie.includes("hotnews_session")){console.log("[TopicTracker] No session cookie, skipping topic tabs");return}let t=document.querySelector(".category-tabs");if(!t){console.warn("[TopicTracker] category-tabs not found");return}let o=Ae(t);try{console.log("[TopicTracker] Loading topics from API...");let c=new AbortController,n=setTimeout(()=>c.abort(),8e3),s=await fetch("/api/topics",{credentials:"include",signal:c.signal});if(clearTimeout(n),o&&o.remove(),s.status===401){console.log("[TopicTracker] Not authenticated, skipping"),x=[];return}let a=await s.json();a.ok&&a.topics?.length>0?(x=a.topics,console.log(`[TopicTracker] Loaded ${x.length} topics, rendering tabs...`),ye(x),ge()):(console.log("[TopicTracker] No topics found or API error:",a.detail||a.error),x=[])}catch(c){c.name==="AbortError"?console.warn("[TopicTracker] Topics API timeout, will retry..."):console.error("[TopicTracker] Failed to load topics:",c),o&&o.remove(),x=[],setTimeout(()=>{console.log("[TopicTracker] Retrying topic tabs load..."),Le()},3e3)}}async function Le(){if(document.querySelector(".category-tabs")&&!document.querySelector(".category-tab.topic-tab"))try{let t=await fetch("/api/topics",{credentials:"include"});if(t.status===401)return;let o=await t.json();o.ok&&o.topics?.length>0&&(x=o.topics,ye(x),ge(),console.log(`[TopicTracker] Retry succeeded, loaded ${x.length} topics`))}catch(t){console.error("[TopicTracker] Retry failed:",t)}}function ge(){try{let e=localStorage.getItem("hotnews_active_tab"),t=localStorage.getItem("tr_active_tab"),o=e&&e.startsWith("topic-")?e:t;if(o&&o.startsWith("topic-")){let c=document.querySelector(`.category-tab[data-category="${o}"]`),n=document.getElementById(`tab-${o}`);if(c&&n)console.log(`[TopicTracker] Restoring to topic tab: ${o}`),H.switchTab(o);else{console.log(`[TopicTracker] Topic tab not found, clearing saved tab: ${o}`),localStorage.removeItem("tr_active_tab");try{R.consumeNavigationState?.()}catch{}H.switchTab("my-tags")}}}catch(e){console.error("[TopicTracker] Failed to restore topic tab:",e)}}function Ae(e){let t=e.querySelector('[data-category="my-tags"]'),o=document.createElement("div");o.className="topic-tabs-loading",o.id="topicTabsSkeleton";for(let c=0;c<2;c++){let n=document.createElement("div");n.className="topic-tab-skeleton",n.innerHTML=`
                <div class="skeleton-icon"></div>
                <div class="skeleton-text"></div>
            `,o.appendChild(n)}if(t)e.insertBefore(o,t);else{let c=document.getElementById("newTopicBtn");c&&c.nextSibling?e.insertBefore(o,c.nextSibling):e.appendChild(o)}return o}function ye(e){let t=document.querySelector(".category-tabs");if(!t){console.warn("[TopicTracker] category-tabs not found");return}let o=t.querySelector('[data-category="my-tags"]');e.forEach(c=>{let n=`topic-${c.id}`;if(document.querySelector(`[data-category="${n}"]`)){console.log(`[TopicTracker] Tab ${n} already exists, skipping`);return}let s=document.createElement("div");if(s.className="category-tab topic-tab",s.dataset.category=n,s.dataset.topicId=c.id,s.draggable=!1,s.onclick=()=>{H.switchTab(n)},s.innerHTML=`
        ...
        `,o.appendChild(c)}function Re(){document.body.insertAdjacentHTML("beforeend",`
        ...
        `),W=document.getElementById("topicModalOverlay"),D=document.getElementById("topicModalTitle"),_=document.getElementById("topicNameInput"),he=document.getElementById("topicKeywordsContainer"),B=document.getElementById("topicSourcesContainer"),E=document.getElementById("topicAiBtn"),v=document.getElementById("topicSubmitBtn"),console.log("[TopicTracker] Modal created, elements cached:",{modalOverlay:!!W,modalTitle:!!D,topicNameInput:!!_})}function z(){let e=document.querySelector(".category-tabs");if(!e){console.warn("[TopicTracker] category-tabs not found, will retry later"),setTimeout(z,1e3);return}if(document.getElementById("newTopicBtn"))return;let t=document.createElement("div");t.className="category-tab new-topic-tab",t.id="newTopicBtn",t.innerHTML=`
            <div class="category-tab-icon">\u2795</div>
            <div class="category-tab-name">\u65B0\u5EFA\u4E3B\u9898</div>
        `,t.onclick=function(){TopicTracker.openModal()},e.insertBefore(t,e.firstChild),console.log("[TopicTracker] New topic button added to tabs");let o=new MutationObserver(c=>{document.getElementById("newTopicBtn")||(console.log("[TopicTracker] Button removed, re-adding..."),o.disconnect(),setTimeout(()=>{z(),ve()},100))});o.observe(e,{childList:!0,subtree:!1})}async function He(){await O()}function ve(){document.querySelectorAll('.category-tab[data-category^="topic-"]').forEach(e=>{let o=e.dataset.category.replace("topic-","");e.addEventListener("click",()=>{setTimeout(()=>P(o),100)}),e.addEventListener("touchstart",()=>{setTimeout(()=>P(o),100)},{passive:!0})}),document.querySelectorAll('.tab-pane[id^="tab-topic-"]').forEach(e=>{let t=e.id.replace("tab-topic-","");new MutationObserver(c=>{for(let n of c)if(n.type==="attributes"&&n.attributeName==="class"&&e.classList.contains("active")){let s=te(t);!s.loading&&!s.loaded&&P(t)}}).observe(e,{attributes:!0,attributeFilter:["class"]})})}function P(e){let t=te(e);if(t.loading){console.log(`[TopicTracker] Topic ${e} already loading, skipping`);return}if(t.loaded){console.log(`[TopicTracker] Topic ${e} already loaded, skipping`);return}let o=document.getElementById(`topicCards-${e}`);if(!o){console.log(`[TopicTracker] Container topicCards-${e} not found`);return}if(o.querySelector(".news-item")){console.log(`[TopicTracker] Topic ${e} already has real content, marking as loaded`),t.loaded=!0;return}U(e)}async function U(e,t=!1){let o=te(e);if(o.loading){console.log(`[TopicTracker] Topic ${e} already loading, skipping`);return}if(o.loaded&&!t){console.log(`[TopicTracker] Topic ${e} already loaded, skipping`);return}let c=document.getElementById(`topicCards-${e}`);if(!c){console.error(`[TopicTracker] Container topicCards-${e} not found`);return}o.loading=!0,console.log(`[TopicTracker] Loading news for topic ${e}...`),Pe(c);try{let n=new AbortController,s=setTimeout(()=>n.abort(),15e3),a=`/api/topics/${e}/news?limit=50`;console.log(`[TopicTracker] Fetching: ${a}`);let i=await fetch(a,{credentials:"include",signal:n.signal});if(clearTimeout(s),console.log(`[TopicTracker] API response status: ${i.status}`),!i.ok){if(i.status===401){Ne(c,e),o.loading=!1;return}if(i.status===404){console.log(`[TopicTracker] Topic ${e} not found (404), removing tab`);let p=`topic-${e}`,u=document.querySelector(`.category-tab[data-category="${p}"]`);u&&u.remove();let l=document.getElementById(`tab-${p}`);l&&l.remove();try{localStorage.getItem("tr_active_tab")===p&&localStorage.removeItem("tr_active_tab")}catch{}H.switchTab("my-tags"),o.loading=!1;return}throw new Error(`HTTP ${i.status}`)}let r=await i.json();if(console.log("[TopicTracker] API response:",{ok:r.ok,hasKeywordsNews:!!r.keywords_news,hasSourcesNews:!!r.sources_news,cached:r.cached}),r.ok){if(We(e,r.keywords_news,r.sources_news),o.loaded=!0,Q>0||window._trNoRebuildExpected)try{let p=R.peekNavigationState?.()||null,u=`topic-${e}`;if(p&&p.activeTab===u){console.log(`[TopicTracker] Restoring navigation scroll after topic news loaded (gen: ${Q}): ${u}`);let l=R.consumeNavigationState();requestAnimationFrame(()=>{R.restoreNavigationScrollY(l||p),R.restoreNavGridScroll(l||p)})}}catch(p){console.error("[TopicTracker] Failed to restore scroll after news load:",p)}else console.log(`[TopicTracker] Skipping scroll restore on initial load (gen: ${Q})`);r.cached&&console.log(`[TopicTracker] Loaded from cache (age: ${r.cache_age}s)`)}else throw new Error(r.error||"\u52A0\u8F7D\u5931\u8D25")}catch(n){console.error(`[TopicTracker] Load topic ${e} failed:`,n);let s=n.name==="AbortError"?"\u8BF7\u6C42\u8D85\u65F6\uFF0C\u8BF7\u91CD\u8BD5":n.message||"\u52A0\u8F7D\u5931\u8D25";je(c,s,e)}finally{o.loading=!1}}function Pe(e){e.innerHTML=`
            <div class="topic-loading-state" style="text-align:center;padding:60px 20px;color:#6b7280;width:100%;">
                <div style="font-size:48px;margin-bottom:16px;">\u{1F50D}</div>
                <div style="font-size:16px;">\u52A0\u8F7D\u4E2D...</div>
            </div>
        ...
        `}function Qe(e){oe(e),U(e,!0)}function We(e,t,o){let c=document.getElementById(`topicCards-${e}`);if(!c){console.error(`[TopicTracker] renderTopicNews: container topicCards-${e} not found`);return}let n=Object.keys(t||{}),s=Object.keys(o||{});console.log(`[TopicTracker] renderTopicNews: topicId=${e}, keywords=`,n,", sources=",s);let a=n.filter(p=>(t[p]||[]).length>0).map(p=>{let u=t[p]||[];return Oe(p,u)}).join(""),i=s.filter(p=>(o[p]||[]).length>0).map(p=>{let u=o[p]||[];return Fe(p,u)}).join(""),r="";if(a&&(r+=a),i&&(r+=i),!r)c.innerHTML=`
            ...
            `;else{c.innerHTML=r;let p=n.filter(l=>(t[l]||[]).length>0).length,u=s.filter(l=>(o[l]||[]).length>0).length;console.log(`[TopicTracker] renderTopicNews: rendered ${p} keyword cards, ${u} source cards`)}}function Fe(e,t){let o=t[0]?.source||"",c=t.length>0?t.slice(0,50).map((i,r)=>{let p=i.id||`source-${Date.now()}-${r}`,u=m(i.title||""),l=m(i.url||""),h=u.replace(/'/g,"\\'").replace(/"/g,"&quot;"),y=l.replace(/'/g,"\\'").replace(/"/g,"&quot;"),$=`source-${e}`,T=m(e).replace(/'/g,"\\'"),g=`<button class="news-summary-btn" data-news-id="${p}" data-title="${u}" data-url="${l}" data-source-id="${$}" data-source-name="${T}" onclick="event.preventDefault();event.stopPropagation();handleSummaryClick(event, '${p}', '${h}', '${y}', '${$}', '${T}')"></button>`,f=`<button class="news-comment-btn" data-url="${l}" data-title="${u}"></button>`;return`
                ...
                <li class="news-item" data-news-id="${a}" data-news-title="${i}" data-news-url="${r}">
                    <div class="news-item-content">
                        ...
                        <div class="news-actions">
                            ...
                            <div class="news-hover-btns">${y}${$}</div>
                        </div>
                    </div>
                </li>
        ...
        `}function be(){return!!J.getUser()}function Te(){typeof window.openLoginModal=="function"?window.openLoginModal():alert("\u8BF7\u5148\u767B\u5F55")}function De(){if(!be()){Te();return}w=null,d=null,D.textContent="\u65B0\u5EFA\u8FFD\u8E2A\u4E3B\u9898",_.value="",document.getElementById("topicIconGroup").style.display="none",document.getElementById("topicKeywordsGroup").style.display="none",document.getElementById("topicSourcesGroup").style.display="none",E.style.display="",E.disabled=!1,E.innerHTML="\u{1F916} \u751F\u6210\u5173\u952E\u8BCD\u548C\u63A8\u8350\u6E90",v.style.display="none",W.classList.add("active"),_.focus()}function V(){W.classList.remove("active"),w=null,d=null}function pe(e,t){return new Promise(o=>{let c=`
        ...
        `;try{let o=await(await fetch("/api/wechat/auth/qr/start",{method:"POST",credentials:"include"})).json();if(!o.ok)throw new Error(o.error||"\u521B\u5EFA\u4F1A\u8BDD\u5931\u8D25");q.sessionId=o.session_id;let c=`/api/wechat/auth/qr/image?t=${Date.now()}`;e.innerHTML=`
            ...
            `}).join("")+`
            <button class="topic-keyword-add" onclick="TopicTracker.addKeyword()">+ \u6DFB\u52A0\u5173\u952E\u8BCD</button>
        `;he.innerHTML=t}function Ve(){let e=prompt("\u8BF7\u8F93\u5165\u5173\u952E\u8BCD\uFF1A");!e||!e.trim()||(d||(d={icon:"\u{1F3F7}\uFE0F",keywords:[],recommended_sources:[]},document.getElementById("topicIconGroup").style.display="block",document.getElementById("topicKeywordsGroup").style.display="block"),d.keywords.includes(e.trim())||(d.keywords.push(e.trim()),ne(),v.disabled=!1))}function Xe(e){d&&(d.keywords=d.keywords.filter(t=>t!==e),ne(),d.keywords.length===0&&(v.disabled=!0))}function ae(){if(!d||!d.recommended_sources)return;let e=d.recommended_sources;if(e.length===0){B.innerHTML=`
        ...
        `,B.innerHTML=t}function Ze(){document.querySelectorAll(".topic-source-checkbox").forEach(e=>e.checked=!0)}function et(){document.querySelectorAll(".topic-source-checkbox").forEach(e=>e.checked=!1)}async function tt(){let e=_.value.trim();if(!e){alert("\u8BF7\u8F93\u5165\u4E3B\u9898\u540D\u79F0");return}if(!d||d.keywords.length===0){alert("\u8BF7\u81F3\u5C11\u6DFB\u52A0\u4E00\u4E2A\u5173\u952E\u8BCD");return}let t=[];document.querySelectorAll(".topic-source-checkbox:checked:not(.existing-source)").forEach(c=>{let n=parseInt(c.dataset.sourceIdx);d.recommended_sources&&d.recommended_sources[n]&&t.push(d.recommended_sources[n])});let o=[];document.querySelectorAll(".topic-source-checkbox.existing-source:checked").forEach(c=>{let n=c.dataset.sourceId;n&&o.push(n)}),v.disabled=!0,v.innerHTML='<span class="topic-spinner"></span> \u521B\u5EFA\u4E2D...';try{let c=[],n=[];document.querySelectorAll(".topic-source-checkbox:checked:not(.existing-source)").forEach(h=>{let y=parseInt(h.dataset.sourceIdx),$=h.dataset.sourceId;if(d.recommended_sources&&d.recommended_sources[y]){let T=d.recommended_sources[y];T.verified&&T.id?c.push(T.id):n.push(T)}});let s=[];if(n.length>0){v.innerHTML='<span class="topic-spinner"></span> \u9A8C\u8BC1\u6570\u636E\u6E90...';let y=await(await fetch("/api/topics/validate-sources",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({sources:n})})).json();if(y.ok&&y.validated_sources){s=y.validated_sources.map(f=>f.id);let $=y.validated_sources.filter(f=>f.status==="created").length,T=y.validated_sources.filter(f=>f.status==="exists").length,g=y.failed_sources||[];if(g.length>0){let f=g.map(I=>`\u2022 ${I.name}: ${I.reason}`).join(`
`),k=$+T;k>0?alert(`\u6210\u529F\u6DFB\u52A0 ${k} \u4E2A\u6570\u636E\u6E90

\u4EE5\u4E0B ${g.length} \u4E2A\u6E90\u9A8C\u8BC1\u5931\u8D25\uFF1A
${f}`):c.length===0&&o.length===0&&alert(`\u6240\u6709\u6570\u636E\u6E90\u9A8C\u8BC1\u5931\u8D25\uFF1A
${f}

\u4E3B\u9898\u4ECD\u4F1A\u521B\u5EFA\uFF0C\u4F46\u4E0D\u4F1A\u5173\u8054\u8FD9\u4E9B\u6570\u636E\u6E90\u3002`)}}}let a=[...new Set([...o,...c,...s])];console.log(`[TopicTracker] Source IDs: existing=${o.length}, verified=${c.length}, new=${s.length}, total=${a.length}`),v.innerHTML='<span class="topic-spinner"></span> \u4FDD\u5B58\u4E3B\u9898...';let i={name:e,icon:d.icon,keywords:d.keywords,rss_source_ids:a},r=w?`/api/topics/${w.id}`:"/api/topics",l=await(await fetch(r,{method:w?"PUT":"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(i)})).json();if(l.ok){let h=l.topic?.id||(w?w.id:null);if(!w&&h){let y=a.length;if(y>0){let T=document.getElementById("topicFetchProgress"),g=T?.querySelector(".topic-fetch-progress-text"),f=T?.querySelector(".topic-fetch-progress-fill");T&&(T.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u4E2D...';let k=5;f&&(f.style.width="5%"),g&&(g.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${y} \u4E2A\u6570\u636E\u6E90...`);let I=[{at:20,text:"\u{1F4F1} \u6B63\u5728\u6293\u53D6\u516C\u4F17\u53F7\u6587\u7AE0..."},{at:40,text:"\u{1F4F0} \u6B63\u5728\u83B7\u53D6 RSS \u5185\u5BB9..."},{at:60,text:"\u{1F4BE} \u6B63\u5728\u4FDD\u5B58\u5230\u6570\u636E\u5E93..."},{at:75,text:"\u{1F4CA} \u6B63\u5728\u6574\u7406\u6587\u7AE0\u5217\u8868..."}],A=0,j=setInterval(()=>{if(k<90){k+=(90-k)*.06;let M=Math.round(k);f&&(f.style.width=M+"%"),A<I.length&&M>=I[A].at&&(g&&(g.textContent=I[A].text),A++)}},500);try{let C=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",credentials:"include"})).json();if(clearInterval(j),C.ok){let L=C.fetched||0;f&&(f.style.width="100%"),g&&(g.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${L}\u6761\u65B0\u95FB)`),v.innerHTML='<span class="topic-spinner"></span> \u5904\u7406\u4E2D...',console.log(`[TopicTracker] Fetch completed: ${L} articles from ${y} sources`);let F=C.inactive_sources||[];F.length>0&&await pe(F,h)&&(g&&(g.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await ue(h,F,a))}else console.warn("[TopicTracker] Fetch sources warning:",C.error),f&&(f.style.width="100%"),g&&(g.textContent="\u2705 \u6293\u53D6\u5B8C\u6210\uFF0C\u5373\u5C06\u8DF3\u8F6C...");await new Promise(L=>setTimeout(L,800))}catch(M){console.error("[TopicTracker] Fetch sources failed:",M),clearInterval(j)}}V();let $=`topic-${h}`;try{localStorage.setItem("tr_active_tab",$)}catch{}window.location.reload()}else if(h){let y=new Set(w.rss_source_ids||w.rss_sources||[]),$=a.filter(g=>!y.has(g));if($.length>0){let g=document.getElementById("topicFetchProgress"),f=g?.querySelector(".topic-fetch-progress-text"),k=g?.querySelector(".topic-fetch-progress-fill");g&&(g.style.display="block"),v.innerHTML='<span class="topic-spinner"></span> \u6293\u53D6\u65B0\u6570\u636E\u6E90...';let I=5;k&&(k.style.width="5%"),f&&(f.textContent=`\u{1F4E1} \u6B63\u5728\u6293\u53D6 ${$.length} \u4E2A\u65B0\u6570\u636E\u6E90...`);let A=setInterval(()=>{I<90&&(I+=(90-I)*.08,k&&(k.style.width=Math.round(I)+"%"))},500);try{let M=await(await fetch(`/api/topics/${h}/fetch-sources`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({source_ids:$})})).json();if(clearInterval(A),M.ok){let C=M.fetched||0;k&&(k.style.width="100%"),f&&(f.textContent=`\u2705 \u6293\u53D6\u5B8C\u6210 (${C}\u6761\u65B0\u6587\u7AE0)`);let L=M.inactive_sources||[];L.length>0&&await pe(L,h)&&(f&&(f.textContent="\u{1F5D1}\uFE0F \u6B63\u5728\u79FB\u9664\u4E0D\u6D3B\u8DC3\u8D26\u53F7..."),await ue(h,L,a))}else k&&(k.style.width="100%"),f&&(f.textContent="\u2705 \u4FDD\u5B58\u5B8C\u6210");await new Promise(C=>setTimeout(C,600))}catch(j){console.error("[TopicTracker] Edit fetch failed:",j),clearInterval(A)}}V();let T=`topic-${h}`;try{localStorage.setItem("tr_active_tab",T)}catch{}window.location.reload()}}else alert(l.error||"\u64CD\u4F5C\u5931\u8D25")}catch(c){console.error("Submit topic failed:",c),alert("\u64CD\u4F5C\u5931\u8D25\uFF0C\u8BF7\u91CD\u8BD5")}finally{v.disabled=!1,v.innerHTML=w?"\u4FDD\u5B58\u66F4\u6539":"\u521B\u5EFA\u5E76\u5F00\u59CB\u8FFD\u8E2A"}}function ot(e){oe(e),U(e,!0)}async function ct(e){if(!be()){Te();return}let t=x.find(o=>o.id===e);if(t){w=t,d={icon:t.icon||"\u{1F3F7}\uFE0F",keywords:[...t.keywords],recommended_sources:[],keywordStats:{}};try{let c=await(await fetch(`/api/topics/${e}/news?limit=1`,{credentials:"include"})).json();if(c.ok&&c.keywords_news)for(let n of t.keywords)d.keywordStats[n]=(c.keywords_news[n]||[]).length}catch(o){console.warn("Failed to load keyword stats:",o)}D.textContent="\u7F16\u8F91\u4E3B\u9898",_.value=t.name,E.style.display="none",v.style.display="",v.textContent="\u4FDD\u5B58\u66F4\u6539",v.disabled=!1,xe(),document.getElementById("topicSourcesGroup").style.display="block",await ie(t.rss_sources||[]),W.classList.add("active")}}var b={checked:!1,hasAuth:!1,qrPolling:!1,pollTimer:null,sessionId:null};function st(){S=[];let e=`
        ...
        `,t=B.querySelector(".topic-sources-add");t?(t.insertAdjacentHTML("beforebegin",e),t.style.display="none"):B.insertAdjacentHTML("beforeend",e)}function nt(){let e=document.getElementById("mpSearchInput");e&&$e(e.value)}var S=[];async function $e(e){let t=document.getElementById("mpSearchResults");if(!t)return;let o=e.trim();if(o.length<2){t.innerHTML='<div class="topic-mp-empty">\u8BF7\u8F93\u5165\u81F3\u5C112\u4E2A\u5B57\u7B26</div>';return}t.innerHTML='<div class="topic-mp-empty">\u641C\u7D22\u4E2D...</div>';try{let c=await fetch(`/api/wechat/search?keyword=${encodeURIComponent(o)}&limit=20`,{credentials:"include"});if(!c.ok){let a=await c.json().catch(()=>({}));throw new Error(a.detail||"\u641C\u7D22\u5931\u8D25")}let n=await c.json(),s=n.list||n.results||[];if(s.length===0){t.innerHTML='<div class="topic-mp-empty">\u672A\u627E\u5230\u5339\u914D\u7684\u516C\u4F17\u53F7</div>';return}t.innerHTML=s.map(a=>{let i=S.some(r=>r.fakeid===a.fakeid);return`
                ...
                `}).join("")}catch(c){console.error("Search MPs failed:",c),t.innerHTML=`<div class="topic-mp-empty">\u641C\u7D22\u5931\u8D25: ${m(c.message)}</div>`}}function at(e){let t=e.dataset.fakeid,o=e.dataset.nickname,c=S.findIndex(n=>n.fakeid===t);c>=0?(S.splice(c,1),e.classList.remove("selected"),e.querySelector(".topic-mp-check").textContent=""):(S.push({fakeid:t,nickname:o}),e.classList.add("selected"),e.querySelector(".topic-mp-check").textContent="\u2713"),Se()}function Se(){let e=document.getElementById("mpSelectedList");if(e){if(S.length===0){e.innerHTML="";return}e.innerHTML=`
            <div class="topic-mp-selected-label">\u5DF2\u9009\u62E9 ${S.length} \u4E2A\u516C\u4F17\u53F7\uFF1A</div>
        ...
        `,e.style.display="",Me())}async function Me(){let e=document.getElementById("topicWechatQRArea");if(e){e.innerHTML=`
            <div class="topic-wechat-qr-loading">
                <span class="topic-spinner"></span>
                \u6B63\u5728\u83B7\u53D6\u4E8C\u7EF4\u7801...
            </div>
