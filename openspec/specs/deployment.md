...
```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP 'cd ~/hotnews && git pull && systemctl restart hotnews'
```

#### Docker Deployment
```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP 'cd ~/hotnews && git pull && cd docker && docker-compose restart hotnews'
```

#### Direct File Sync (Hot Fix Only)
```bash
# Sync single file
rsync -avz -e "ssh -p YOUR_SSH_PORT" hotnews/web/server.py root@YOUR_SERVER_IP:~/hotnews/hotnews/web/

# Restart service
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP 'systemctl restart hotnews'
```
...
curl http://YOUR_SERVER_IP:8080/api/news | python3 -m json.tool | grep -A 2 "掘金"

# Web interface
open http://YOUR_SERVER_IP:8080/viewer
```
...
If deployment fails:
```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP 'cd ~/hotnews && git reset --hard HEAD~1 && systemctl restart hotnews'
```
...
- Production server: `root@YOUR_SERVER_IP:YOUR_SSH_PORT`, project path `~/hotnews`.
...
```bash
gzip -dc V1.0.1.tar.gz | ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP "docker load"
```

2) Switch the server to the target version and start services:

```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP "cd ~/hotnews/docker && \
...
```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP "curl -fsS http://127.0.0.1:8090/health && echo"
...
```bash
ssh -p YOUR_SSH_PORT root@YOUR_SERVER_IP "tar -czf - -C ~/hotnews config output" > hotnews-data-$(date +%F).tar.gz
```
